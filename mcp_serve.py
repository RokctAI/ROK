"""
Rok MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Rok-specific extra)

Usage:
    rok mcp serve
    rok mcp serve --verbose
...
logger = logging.getLogger("rok.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ROK_HOME."""
    try:
        from rok_constants import get_rok_home
        return get_rok_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ROK_HOME", Path.home() / ".rok")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from rok_constants import get_rok_home
        directory_file = get_rok_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ROK_HOME", Path.home() / ".rok")
...
class EventBridge:
    ...
    This is the Rok equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from rok_constants import get_rok_home
            db_file = get_rok_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ROK_HOME", Path.home() / ".rok")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Rok MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'rok-agent[mcp]'"
        )

    mcp = FastMCP(
        "rok",
        instructions=(
            "Rok Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Rok MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
