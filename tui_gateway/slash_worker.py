"""Persistent slash-command worker — one Rokcli per TUI session.
...
def _run(cli: Rokcli, command: str) -> str:
...
def main():
    ...
    os.environ["ROK_SESSION_KEY"] = args.session_key
    os.environ["ROK_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = Rokcli(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
