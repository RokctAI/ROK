...
# into ~/.rok/node/ for use by Rok Agent's browser tools.
#
# Targets the registry-install path: users who got Rok via
# `uvx --from 'rok-agent[acp]==X' rok-acp` don't have a repo clone,
...
# safe to run from `rok-acp --setup-browser`, from a fresh terminal,
...
#   ROK_HOME=/custom/path bootstrap_browser_tools.sh
#
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -h|--help)
            cat <<EOF
Bootstrap Rok Agent browser tools.

Installs Node.js (into ~/.rok/node/), the agent-browser npm package,
...
Environment:
  ROK_HOME       Override Rok data dir (default: \$HOME/.rok)
EOF
            exit 0
            ;;
        *)
...
ensure_node() {
    ...
    if [ -x "$NODE_PREFIX/bin/node" ]; then
        ...
        log_success "Node.js $found_ver found (Rok-managed at $NODE_PREFIX)"
        return 0
    fi
...
ensure_agent_browser() {
    ...
    # _find_agent_browser() in tools/browser_tool.py walks ~/.rok/node/bin
    ...
    # sudo. Force the prefix to the user-writable Rok-managed Node
...
main() {
    log_info "Rok Agent: bootstrapping browser tools"
    log_info "  ROK_HOME = $ROK_HOME"
    ...
    log_info "Rok Agent will pick up agent-browser from $NODE_PREFIX/bin/ on next launch."
}
