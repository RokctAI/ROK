...
def _load_env() -> None:
    """Load .env from ROK_HOME (default ``~/.rok``)."""
    from rok_cli.env_loader import load_rok_dotenv

    rok_home = get_rok_home()
    loaded = load_rok_dotenv(rok_home=rok_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", rok_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="rok-acp",
        description="Run Rok Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Rok version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import RokACPAgent  # noqa: F401

    print("Rok ACP check OK")


def _run_setup() -> None:
    from rok_cli.main import main as rok_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "rok", "model"]
        rok_main()
    finally:
        sys.argv = old_argv
