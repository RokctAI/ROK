"""ACP session manager — maps ACP sessions to Rok AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.rok/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Rok itself is running in WSL.

    Windows ACP clients can launch ``rok acp`` inside WSL while still sending
    ...
    """
    from rok_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Rok from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["rok-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Rok agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Rok AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Rok runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.rok/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``ROK_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_rok_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=rok_home / "state.db")
            return self._db_instance
        except Exception:
