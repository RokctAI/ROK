...
# 5. Symlinks the 'rok' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Rok Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/rok-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink rok into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up rok command..."

ROK_BIN="$SCRIPT_DIR/venv/bin/rok"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ROK_BIN" "$COMMAND_LINK_DIR/rok"
echo -e "${GREEN}✓${NC} Symlinked rok → $COMMAND_LINK_DISPLAY_DIR/rok"
...
# ============================================================================
# Seed bundled skills into ~/.rok/skills/
# ============================================================================

ROK_SKILLS_DIR="${ROK_HOME:-$HOME/.rok}/skills"
mkdir -p "$ROK_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.rok/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ROK_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  rok status        # Check configuration"
if is_termux; then
    echo "  rok gateway       # Run gateway in foreground"
else
    echo "  rok gateway install # Install gateway service (messaging + cron)"
fi
echo "  rok cron list     # View scheduled jobs"
echo "  rok doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m rok_cli.main setup
fi
