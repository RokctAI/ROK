...
//
// Both gate on ROK_DEV_PERF=1 and dump JSON-lines (default ~/.rok/perf.log,
// override ROK_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// ROK_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
import type { FrameEvent } from '@rok/ink'
import { scrollFastPathStats } from '@rok/ink'
import { Profiler, type ProfilerOnRenderCallback, type ReactNode } from 'react'

const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.ROK_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.ROK_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.ROK_DEV_PERF_LOG?.trim() || join(homedir(), '.rok', 'perf.log')
