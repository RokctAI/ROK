import type { RunExternalProcess } from '@rok/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`rok ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching rok: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`rok ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
