import { useApp, useHasSelection, useSelection, useStdout, useTerminalTitle, type ScrollBoxHandle } from '@rok/ink'
...
export function useMainApp(gw: GatewayClient) {
  ...
  useTerminalTitle(model ? `${marker} ${model}${tabCwd ? ` · ${shortCwd(tabCwd, 24)}` : ''}` : 'Rok')
  ...
  const cwd = ui.info?.cwd || process.env.ROK_CWD || process.cwd()
