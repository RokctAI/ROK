...
React + Ink terminal UI for Rok. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ROK_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.rok/.rok_history` or under `ROK_HOME`.
...
```text
ui-tui/
  packages/rok-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      rok-ink.d.ts    type declarations for @rok/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent Rokcli subprocess for slash commands
```
