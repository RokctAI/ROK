...
const PROVIDER_GROUPS: { prefix: string; name: string; priority: number }[] = [
  ...
  { prefix: "ROK_QWEN_", name: "DashScope (Qwen)", priority: 2 },
      ...
      {isEditing && (
        <div className="flex items-center gap-2">
          <Input
            ...
            placeholder={
              info.is_set
                ? `Replace current value (${info.redacted_value ?? "---"})`
                : "Enter value..."
            }
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          <p className="text-sm text-muted-foreground">
            Manage API keys and secrets stored in <code>~/.rok/.env</code>
          </p>
          <p className="text-[0.7rem] text-muted-foreground/70">
            Changes are saved to disk immediately. Active sessions pick up new
            keys automatically.
          </p>
        </div>
        <Button
