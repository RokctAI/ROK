# Contributing to Rok Agent

Thank you for contributing to Rok Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Rok to work everywhere.
...
Bundled skills (in `skills/`) ship with every Rok install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `rok skills browse` (labeled "official") and install it with `rok skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `rok skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.rok/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(rok_home, config)` for setup-wizard integration
...
- Integrate with `rok memory setup` via `post_setup()` — no need to touch core code
...
```bash
mkdir -p ~/.rok/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.rok/config.yaml
touch ~/.rok/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.rok/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/rok" ~/.local/bin/rok
...
│
├── rok_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.rok/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (rok.rokct.ai)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.rok/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.rok/`)

| Path | Purpose |
|------|---------|
| `~/.rok/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.rok/.env` | API keys and secrets |
| `~/.rok/auth.json` | OAuth credentials (Nous Portal) |
| `~/.rok/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.rok/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.rok/state.db` | SQLite session database |
| `~/.rok/sessions/` | JSON session logs |
| `~/.rok/cron/` | Scheduled job data |
| `~/.rok/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`rok_state.py`) with full-text search and unique session titles. JSON logs go to `~/.rok/sessions/`.
...
(for example `_ROK_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.rok`:
...
The user may skip setup and keep loading the skill. Rok only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `rok setup` or update `~/.rok/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Rok tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Rok Agent" is the secondary collaborator. If the contributor's commit shows "Rok Agent" as author because they used Rok to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Rok tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `rok --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Rok uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.rok/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `rok_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `rok_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Rok runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the rok wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `rok_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `rok_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `rok_cli/gateway_windows.py::
...
Rok has terminal access. Security matters.
...
2. **Test manually**: Run `rok` and exercise the code path you changed
