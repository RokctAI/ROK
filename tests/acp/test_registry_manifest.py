"""Tests for ACP Registry metadata shipped with Rok."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "rok-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Rok Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/RokctAI/rok-agent"
    assert data["website"].startswith("https://rok.rokct.ai/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"rok-agent[acp]=={data['version']}"
    assert uvx["args"] == ["rok-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"rok-agent[acp]=={_pyproject_version()}"
    )
