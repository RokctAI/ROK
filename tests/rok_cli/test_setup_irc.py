"""Tests for IRC gateway configuration via `rok setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Rok install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Rok install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#rok")
            monkeypatch.setenv("IRC_NICKNAME", "rok-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `rok setup gateway` experience with IRC present."""
