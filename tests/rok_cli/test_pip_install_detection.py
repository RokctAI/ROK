...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("rok_cli.config.get_managed_system", return_value=None):
...
def test_managed_install_takes_precedence(tmp_path):
    """When ROK_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("rok_cli.config.get_managed_system", return_value="NixOS"):
        from rok_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from rok_cli.config import recommended_update_command_for_method
