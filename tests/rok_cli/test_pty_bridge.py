"""Unit tests for rok_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf rok-ok"])
        try:
            output = _read_until(bridge, b"rok-ok")
            assert b"rok-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$ROK_PTY_TEST\""],
            env={**os.environ, "ROK_PTY_TEST": "pty-env-works"},
        )
        try:
