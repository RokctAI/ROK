"""Tests for the top-level `./rok` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./rok` should use `rok_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "rok"
    called = []

    fake_main_module = types.ModuleType("rok_cli.main")

    def fake_main():
        called.append("rok_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "rok_cli.main", fake_main_module)
