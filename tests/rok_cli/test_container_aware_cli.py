...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ROK_HOME with .container-mode file."""
    rok_home = tmp_path / ".rok"
    rok_home.mkdir()
    monkeypatch.setenv("ROK_HOME", str(rok_home))
    monkeypatch.delenv("ROK_DEV", raising=False)

    container_mode = rok_home / ".container-mode"
...
def test_get_container_exec_info_none_inside_container(container_env):
    ...
    with patch("rok_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_rok_dev(container_env, monkeypatch):
    """Returns None when ROK_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ROK_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_rok_dev_zero(container_env, monkeypatch):
    """ROK_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ROK_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        rok_home = Path(tmpdir) / ".rok"
        rok_home.mkdir()
        (rok_home / ".container-mode").write_text(
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=rok-custom\n"
        "exec_user=myuser\n"
        "rok_bin=/opt/rok/bin/rok\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "rok-custom"
    assert info["exec_user"] == "myuser"
    assert info["rok_bin"] == "/opt/rok/bin/rok"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from rok_cli.main import _exec_in_container
