...
@pytest.fixture()
def rok_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Rok auth store (no Codex CLI needed)."""
    rok_home = tmp_path / ".rok"
    rok_home.mkdir()
...
def test_normal_path_still_works(rok_auth_only_env):
    """openai-codex appears when tokens are already in Rok auth store."""
    from rok_cli.model_switch import list_authenticated_providers
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Rok
    auth store."""
    rok_home = tmp_path / ".rok"
    rok_home.mkdir()
