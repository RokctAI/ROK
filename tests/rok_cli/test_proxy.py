"""Tests for the `rok proxy` subcommand and its upstream adapters."""
...
from rok_cli.proxy.adapters import ADAPTERS, get_adapter
from rok_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from rok_cli.proxy.adapters.nous_portal import NousPortalAdapter
...
def _write_auth_store(rok_home: Path, nous_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given nous state into a hermetic ROK_HOME."""
...
def test_nous_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # ROK_HOME is already set by conftest, but make doubly sure
...
def test_nous_adapter_authenticated_with_agent_key(tmp_path, monkeypatch):
    ...
    _write_auth_store(tmp_path, {
        ...
        "inference_base_url": "https://inference-api.rokctai.com/v1",
    })
...
def test_nous_adapter_get_credential_refreshes_and_persists(tmp_path, monkeypatch):
    ...
    with patch(
        "rok_cli.proxy.adapters.nous_portal.refresh_nous_oauth_from_state",
    ...
    assert cred.base_url == "https://inference-api.rokctai.com/v1"
...
def test_nous_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("ROK_HOME", str(tmp_path))
    adapter = NousPortalAdapter()
    with pytest.raises(RuntimeError, match="rok login nous"):
...
from rok_cli.proxy.server import create_app  # noqa: E402
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("ROK_HOME", str(tmp_path))
    from rok_cli.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from rok_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from rok_cli.proxy.cli import cmd_proxy_start
