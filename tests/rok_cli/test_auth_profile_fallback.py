...
When ``ROK_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.rok/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.rok            (has its own auth.json fixture)
    * Profile     -> tmp_path/.rok/profiles/coder   (active, ROK_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ROK_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.rok as the "real home" — which is NOT equal
    # to the ROK_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    rok_home = tmp_path / "classic"
    rok_home.mkdir()
    monkeypatch.setenv("ROK_HOME", str(rok_home))

    _write(rok_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from rok_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: ROK_HOME is set to a custom path that is NOT under
    # ~/.rok/profiles/ — get_default_rok_root() returns ROK_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from rok_cli.auth import read_credential_pool, write_credential_pool
