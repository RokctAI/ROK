...
``rok_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``rok --help``, ``rok version``,
``rok logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``rok --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from rok_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["rok", "--tui", "chat"], "chat"),
        (["rok", "-w", "logs"], "logs"),
        (["rok", "chat", "hello world"], "chat"),
        (["rok", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["rok", "-m", "gpt5", "chat"], "chat"),
        (["rok", "--model", "gpt5", "chat", "hi"], "chat"),
        (["rok", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["rok", "-z", "hello world"], None),
        (["rok", "-z", "hello", "chat"], "chat"),
        (["rok", "--model=gpt5", "chat"], "chat"),     # inline form
        (["rok", "--", "chat"], "chat"),               # -- terminator
        (["rok", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["rok", "some-plugin-cmd"], "some-plugin-cmd"),
        (["rok", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["rok"],                          # bare → chat
        ["rok", "--help"],                # top-level help
        ["rok", "-h"],
        ["rok", "version"],               # known built-in
        ["rok", "logs"],
        ["rok", "gateway", "run"],
        ["rok", "--tui"],
        ["rok", "-w", "--tui"],
        ["rok", "chat", "hi"],
        ["rok", "help"],                  # accepted built-in-ish
        ["rok", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["rok", "meet", "join"],          # potential google_meet plugin
        ["rok", "honcho", "status"],      # potential memory plugin
        ["rok", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``rok help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"rok_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
