"""Tests for the `rok memory reset` CLI command.
...
- Profile-scoped reset (uses ROK_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ROK_HOME with memory files."""
    rok_home = tmp_path / ".rok"
    memories = rok_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ROK_HOME", str(rok_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nRok repo is at ~/.rok/rok-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `rok memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        rok_home = tmp_path / ".rok"
        (rok_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ROK_HOME", str(rok_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ROK_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        rok_home = tmp_path / ".rok"
        rok_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("ROK_HOME", str(rok_home))

        # The memories dir won't exist; get_rok_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
