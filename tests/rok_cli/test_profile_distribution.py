"""Tests for rok_cli.profile_distribution — git-based profile installs.
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/rok_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_rok_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Rok"):
                check_rok_requires(spec, cur)
...
class TestEnvTemplate:
    ...
    def test_empty_env_requires_is_header_only(self):
        m = DistributionManifest(name="x")
        out = _env_template_from_manifest(m)
        assert "Rok distribution" in out
...
class TestInstall:
    ...
    def test_install_enforces_rok_requires(self, profile_env, monkeypatch):
        # Pin current Rok version to something well below the requirement
        import rok_cli
        monkeypatch.setattr(rok_cli, "__version__", "0.1.0", raising=False)
...
class TestUpdate:
    ...
    def test_update_missing_manifest_errors(self, profile_env):
        ...
        from rok_cli.profiles import create_profile
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        from rok_cli.profiles import get_profile_dir
        ...
        monkeypatch.setattr(
            "rok_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from rok_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_plain_profile_has_no_distribution_fields(self, profile_env):
        from rok_cli.profiles import create_profile, list_profiles
        create_profile(name="plain", no_alias=True)
        rows = {p.name: p for p in list_profiles()}
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from rok_cli.profiles import create_profile, list_profiles, get_profile_dir
