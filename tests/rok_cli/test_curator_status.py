"""Tests for `rok curator status` output.
...
def test_status_uses_last_activity_not_only_last_used(monkeypatch, capsys):
    ...
    import rok_cli.curator as curator_cli
...
def curator_status_env(tmp_path, monkeypatch):
    """Isolated ROK_HOME with real agent-created skills on disk."""
    home = tmp_path / ".rok"
    skills = home / "skills"
    ...
    from rok_cli import curator as curator_cli
