...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        rok_home.mkdir()
        (rok_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(rok_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.rok/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        rok_home = tmp_path / ".rok"
        rok_home.mkdir()
        (rok_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        rok_home = tmp_path / ".rok"
        rok_home.mkdir()
        (rok_home / "secret.key").write_text("key")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.rok/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        rok_home = tmp_path / ".rok"
        (rok_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ROK_HOME": str(rok_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.rok")

        assert mounts[0]["container_path"] == "/home/user/.rok/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.rok/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.rok/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ROK_HOME."""
        monkeypatch.setenv("ROK_HOME", str(tmp_path / ".rok"))
        (tmp_path / ".rok").mkdir()

        # Create a sensitive file one level above rok_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside rok_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ROK_HOME must still be registered."""
        rok_home = tmp_path / ".rok"
        rok_home.mkdir()
        monkeypatch.setenv("ROK_HOME", str(rok_home))
        (rok_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_rok_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ROK_HOME must be allowed."""
        rok_home = tmp_path / ".rok"
        rok_home.mkdir()
        subdir = rok_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ROK_HOME pointing outside must be rejected."""
        rok_home = tmp_path / ".rok"
        rok_home.mkdir()
        monkeypatch.setenv("ROK_HOME", str(rok_home))

        # Create a sensitive file outside rok_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside rok_home pointing outside
        symlink = rok_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ROK_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, rok_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ROK_HOME."""
        rok_home = tmp_path / ".rok"
        rok_home.mkdir()
        monkeypatch.setenv("ROK_HOME", str(rok_home))
        ...
        sensitive.write_text("{}")
        self._write_config(rok_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(rok_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ROK_HOME via config must still mount."""
        rok_home = tmp_path / ".rok"
        rok_home.mkdir()
        monkeypatch.setenv("ROK_HOME", str(rok_home))

        (rok_home / "oauth.json").write_text("{}")
        self._write_config(rok_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        rok_home = tmp_path / ".rok"
        rok_home.mkdir()
        (rok_home / "cache" / "documents").mkdir(parents=True)
        (rok_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("ROK_HOME", str(rok_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.rok/cache/documents" in paths
        assert "/root/.rok/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.rok/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        rok_home = tmp_path / ".rok"
        rok_home.mkdir()
        # Use legacy dir name — get_rok_dir prefers old if it exists
        (rok_home / "document_cache").mkdir()
        (rok_home / "image_cache").mkdir()
        monkeypatch.setenv("ROK_HOME", str(rok_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(rok_home / "document_cache") in host_paths
        assert str(rok_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.rok/cache/documents" in container_paths
        assert "/root/.rok/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        rok_home = tmp_path / ".rok"
        doc_dir = rok_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        rok_home = tmp_path / ".rok"
        ss_dir = rok_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.rok/cache/screenshots/session_abc/screen1.png"
