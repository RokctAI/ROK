...
def _restore_tool_modules():
    original_rok_home = os.environ.get("ROK_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_rok_home is None:
            os.environ.pop("ROK_HOME", None)
        else:
            os.environ["ROK_HOME"] = original_rok_home
        _reset_modules(("tools", "rok_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    rok_cli = types.ModuleType("rok_cli")
    rok_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["rok_cli"] = rok_cli
    rok_home = tmp_path / "rok-home"
    os.environ["ROK_HOME"] = str(rok_home)
    sys.modules["rok_cli.config"] = types.SimpleNamespace(
        get_rok_home=lambda: rok_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="rok-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": rok_home / "modal_snapshots.json",
