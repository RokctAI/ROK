...
class TestCronjobRequirements:
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("ROK_INTERACTIVE", false_like_value)
        monkeypatch.delenv("ROK_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("ROK_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["ROK_INTERACTIVE", "ROK_GATEWAY_SESSION", "ROK_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("ROK_INTERACTIVE", "ROK_GATEWAY_SESSION", "ROK_EXEC_ASK"):
