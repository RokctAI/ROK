...
class TestRegisterServerTools:
    ...
    def test_injects_rok_toolsets(self, mock_registry, mock_toolsets):
        """Tools are injected into rok-* toolsets but not custom ones."""
        ...
        # Injected into rok-* toolsets
        assert "mcp_my_srv_my_tool" in mock_toolsets["rok-cli"]["tools"]
        assert "mcp_my_srv_my_tool" in mock_toolsets["rok-telegram"]["tools"]
        # NOT into non-rok toolsets
...
class TestRefreshTools:
    ...
    async def test_nuke_and_repave(self, mock_registry, mock_toolsets):
        ...
        mock_toolsets["rok-cli"]["tools"].append("mcp_live_srv_old_tool")
        ...
        assert "mcp_live_srv_old_tool" not in mock_toolsets["rok-cli"]["tools"]

        # New tool registered
        assert "mcp_live_srv_new_tool" in mock_registry.get_all_tool_names()
        assert "mcp_live_srv_new_tool" in mock_toolsets["rok-cli"]["tools"]
