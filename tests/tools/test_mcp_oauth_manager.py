...
def test_rok_provider_subclass_exists():
    """RokMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ROK_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ROK_PROVIDER_CLS is not None
    assert issubclass(_ROK_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_rok_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns RokMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ROK_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ROK_HOME", str(tmp_path))
    ...
    assert _ROK_PROVIDER_CLS is not None
    assert isinstance(provider, _ROK_PROVIDER_CLS)
    assert provider._rok_server_name == "srv"
