...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._rok_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_rok_bin(self):
        """After _INSTALL_FAILED, manual install in ROK_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        rok_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(rok_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(rok_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._rok_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == rok_bin
            assert _tirith_mod._resolved_path == rok_bin
            mock_clear.assert_called_once()
...
class TestRokHomeIsolation:
    def test_rok_bin_dir_respects_rok_home(self):
        ...
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ROK_HOME": tmpdir}):
            result = _rok_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_rok_home(self):
        """_failure_marker_path must use ROK_HOME, not hardcoded ~/.rok."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ROK_HOME": "/custom/rok"}):
            result = _failure_marker_path()
        assert result == "/custom/rok/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ROK_HOME; verify it's active."""
        rok_home = os.getenv("ROK_HOME")
        assert rok_home is not None, "ROK_HOME should be set by conftest"
        assert "rok_test" in rok_home, "Should point to test temp dir"

    def test_get_rok_home_fallback(self):
        """Without ROK_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_rok_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ROK_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".rok")
            result = _get_rok_home()
