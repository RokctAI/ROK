...
def _isolate(tmp_path, monkeypatch):
    """Isolate ROK_HOME so tests don't touch real config."""
    home = tmp_path / ".rok"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal Rokcli instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = Rokcli.__new__(Rokcli)
        obj.model = "test-model"
        obj.enabled_toolsets = ["rok-core"]
        obj.compact = False
