...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("rok/rok-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned rok/* and pr-* branches."""

    def test_prunes_orphaned_rok_branch(self, git_repo):
        """rok/rok-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "rok/rok-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "rok/rok-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "rok/rok-deadbeef" in result.stdout
        ...
        assert "rok/rok-deadbeef" in orphaned
        ...
        assert "rok/rok-deadbeef" not in result.stdout
