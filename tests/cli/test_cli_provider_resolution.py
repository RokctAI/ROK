...
def test_cli_turn_routing_uses_cheap_model_when_simple(monkeypatch):
    ...
    shell = cli.Rokcli(model="anthropic/claude-sonnet-4", compact=True, max_turns=1)
    shell.provider = "openrouter"
    shell.api_mode = "chat_completions"
    shell.base_url = "https://openrouter.ai/api/v1"
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    monkeypatch.setenv("ROK_ENABLE_NOUS_MANAGED_TOOLS", "1")
    config = {
    ...
    monkeypatch.setattr(
        "rok_cli.nous_subscription.get_nous_subscription_explainer_lines",
