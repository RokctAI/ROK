"""
Tests for timezone support (rok_time module + integration points).
...
class TestRokTimeNow:
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["ROK_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="rok_time"):
            result = rok_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["ROK_TIMEZONE"] = "Europe/London"
    ...
    def test_returns_none_for_invalid(self):
        os.environ["ROK_TIMEZONE"] = "Not/A/Timezone"
        tz = rok_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When ROK_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) ROK_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("ROK_TIMEZONE=" + os.environ.get("ROK_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "ROK_TIMEZONE=NOT_SET" in result["output"], (
            "ROK_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When ROK_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("ROK_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=rok_tz) which shifted
        absolute time when system-local tz != Rok tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_rok_tz(self):
        """Already-aware datetimes should be normalized to Rok tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Rok tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Rok tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Rok is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Rok tz
        ...
        # Use a Rok timezone far behind UTC so that the numeric wall time
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Rok timezone"
        )
