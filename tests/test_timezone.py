...
class TestCodeExecutionTZ:
    ...
    def test_rok_timezone_not_leaked_to_child(self):
        """ROK_TIMEZONE itself must NOT appear in child env (only TZ)."""
        import json as _json
        os.environ["ROK_TIMEZONE"] = "Asia/Kolkata"

        with patch("model_tools.handle_function_call", side_effect=self._mock_handle):
            result = _json.loads(self._execute_code(
                code='import os; print(os.environ.get("ROK_TIMEZONE", "NOT_SET"))',
