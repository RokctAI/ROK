...
def server():
    with patch.dict(
        "sys.modules",
        {
            "rok_constants": MagicMock(
                get_rok_home=MagicMock(return_value="/tmp/rok_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'rok-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'rok-release' patched"
    }
