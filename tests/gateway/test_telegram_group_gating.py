...
def test_group_messages_can_require_direct_trigger_via_config():
    ...
    assert adapter._should_process_message(_group_message("hi @rok_bot", entities=[_mention_entity("hi @rok_bot")])) is True
    ...
    assert adapter._should_process_message(
        _group_message(
            "/status@rok_bot",
            entities=[_bot_command_entity("/status@rok_bot", "/status@rok_bot")],
        ),
        is_command=True,
    ) is True
...
def test_guest_mode_allows_only_direct_mentions_outside_allowed_chats():
    ...
    mentioned = _group_message(
        "hi @rok_bot",
        chat_id=-201,
        entities=[_mention_entity("hi @rok_bot")],
    )
