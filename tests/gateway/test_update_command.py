...
class TestHandleUpdateCommand:
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _rok_home to tmp_path and project_root to a dir without .git
    ...
    @pytest.mark.asyncio
    async def test_no_rok_binary(self, tmp_path):
        """Returns error when rok is not on PATH and rok_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "rok update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m rok_cli.main when rok not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "Starting Rok update" in result
        call_args = mock_popen.call_args[0][0]
        # The update_cmd uses sys.executable -m rok_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "rok_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_rok_bin_prefers_which(self, tmp_path):
        """_resolve_rok_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_rok_bin

        with patch("shutil.which", return_value="/custom/path/rok"):
            result = _resolve_rok_bin()

        assert result == ["/custom/path/rok"]

    @pytest.mark.asyncio
    async def test_resolve_rok_bin_fallback(self):
        """_resolve_rok_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_rok_bin
        ...
        assert result == [sys.executable, "-m", "rok_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_rok_bin_returns_none_when_both_fail(self):
        """_resolve_rok_bin returns None when both strategies fail."""
        from gateway.run import _resolve_rok_bin
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        pending_path = rok_home / ".update_pending.json"
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (rok_home / ".update_pending.json").exists()
        assert not (rok_home / ".update_exit_code").exists()
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (rok_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = rok_home / ".update_pending.claimed.json"
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (rok_home / ".update_output.txt").write_text(
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (rok_home / ".update_pending.json").write_text(json.dumps(pending))
        (rok_home / ".update_output.txt").write_text("x" * 5000)
        (rok_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (rok_home / ".update_pending.json").write_text(json.dumps(pending))
        (rok_home / ".update_output.txt").write_text("Traceback: boom")
        (rok_home / ".update_exit_code").write_text("1")
    ...
    async def test_cleans_up_files_after_notification(self, tmp_path):
        ...
        pending_path = rok_home / ".update_pending.json"
        output_path = rok_home / ".update_output.txt"
