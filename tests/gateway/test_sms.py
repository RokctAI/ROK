...
class TestSmsToolset:
    def test_rok_sms_toolset_exists(self):
        from toolsets import get_toolset

        ts = get_toolset("rok-sms")
        assert ts is not None
        assert "tools" in ts

    def test_rok_sms_in_gateway_includes(self):
        from toolsets import get_toolset

        gw = get_toolset("rok-gateway")
        assert gw is not None
        assert "rok-sms" in gw["includes"]
