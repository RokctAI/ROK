"""
Tests for mcp_serve — Rok MCP server.
...
def _isolate_rok_home(tmp_path, monkeypatch):
    """Redirect ROK_HOME to a temp directory."""
    monkeypatch.setenv("ROK_HOME", str(tmp_path))
    try:
        import rok_constants
        monkeypatch.setattr(rok_constants, "get_rok_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking rok_state schema."""
