...
def test_rok_client_tag_includes_current_version():
    """The client tag must reflect rok_cli.__version__ verbatim."""
    from rok_cli import __version__
    from agent.portal_tags import rok_client_tag

    assert rok_client_tag() == f"client=rok-client-v{__version__}"


def test_rok_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import rok_client_tag

    tag = rok_client_tag()
    assert tag.startswith("client=rok-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import rok_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=rok-agent" in tags
    assert rok_client_tag() in tags
    assert len(tags) == 2
