...
""",
        ...
        from rok_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        rok_home = tmp_path / ".rok"
        rok_home.mkdir()
        (rok_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ROK_HOME", str(rok_home))
        from rok_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from rok_cli.plugins import PluginContext, PluginManifest, PluginManager
