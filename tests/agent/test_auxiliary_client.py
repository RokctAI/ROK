...
class TestExpiredCodexFallback:
    ...
    def test_rok_oauth_file_sets_oauth_flag(self, monkeypatch):
        ...
        with patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="rok-oauth-jwt-token"), \
             patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from agent.auxiliary_client import _try_anthropic, AnthropicAuxiliaryClient
...
class TestExplicitProviderRouting:
    ...
    def test_explicit_google_alias_uses_gemini_credentials(self):
        ...
        with (
            patch("rok_cli.auth.resolve_api_key_provider_credentials", return_value={
                "api_key": "gemini-key",
...
class TestGetTextAuxiliaryClient:
    ...
    def test_custom_endpoint_over_codex(self, monkeypatch, codex_auth_dir):
        ...
        monkeypatch.setattr("rok_cli.config.load_config", lambda: config)
        monkeypatch.setattr("rok_cli.runtime_provider.load_config", lambda: config)
