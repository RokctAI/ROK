...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    rok_state = ModuleType("rok_state")
    rok_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "rok_state", rok_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Rok"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Rok"
