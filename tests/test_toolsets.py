...
class TestToolsetConsistency:
    ...
    def test_rok_platforms_share_core_tools(self):
        """All rok-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on rok-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["rok-cli", "rok-telegram", "rok-discord", "rok-whatsapp", "rok-slack", "rok-signal", "rok-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_rok_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("rok-whatsapp")

    def test_rok_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("rok-api-server")
