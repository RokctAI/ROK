"""Tests for rok_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Rok entry point
(rok, rok-agent, rok-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Rok entry point imports rok_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported rok_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("rok_bootstrap", None)
    import rok_bootstrap  # noqa: WPS433
    return rok_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Rok CLI
...
class TestEntryPointsImportBootstrap:
    """Every Rok entry point must import rok_bootstrap as its
    ...
    # Entry points that invoke Rok as a process.  Each one must
    # import rok_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "rok_cli/main.py",   # rok CLI (console_script)
        "run_agent.py",          # rok-agent (console_script)
        "acp_adapter/entry.py",  # rok-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import rok_bootstrap' and that
        ...
        half-finished ``rok update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``rok_bootstrap`` as a top-level module) leaves rok
        ...
        """
        # Resolve relative to the rok-agent repo root.  Tests live
        # at tests/test_rok_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # rok start even when rok_bootstrap hasn't been
        ...
        assert first_import_name == "rok_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'rok_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import rok_bootstrap' line to be the first import."
        )
