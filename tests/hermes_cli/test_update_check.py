"""Tests for the update check mechanism in rok_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path):
    ...
    with patch("rok_cli.banner.os.getenv", return_value=str(tmp_path)):
...
def test_check_for_updates_expired_cache(tmp_path):
    ...
    with patch("rok_cli.banner.os.getenv", return_value=str(tmp_path)):
        with patch("rok_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
            result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path):
    ...
    fake_banner = tmp_path / "rok_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root():
    """Dev install: falls back to Path(__file__).parent.parent when ROK_HOME has no git repo."""
    import rok_cli.banner as banner
    ...
    with tempfile.TemporaryDirectory() as td:
        with patch("rok_cli.banner.os.getenv", return_value=td):
            with patch("rok_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from rok_cli.main import _invalidate_update_cache

    # Build a fake ~/.rok with default + two named profiles
