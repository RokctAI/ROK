...
- Path resolution (absolute, relative to ROK_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ROK_HOME."""
    rok_home = tmp_path / ".rok"
    rok_home.mkdir()
    (rok_home / "cron").mkdir()
    (rok_home / "cron" / "output").mkdir()
    (rok_home / "scripts").mkdir()
    monkeypatch.setenv("ROK_HOME", str(rok_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ROK_DIR", rok_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", rok_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", rok_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", rok_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.rok/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ROK_SESSION_PLATFORM") is None
        assert os.environ.get("ROK_SESSION_CHAT_ID") is None
        assert os.environ.get("ROK_SESSION_CHAT_NAME") is None
