...
2. **Isolated ROK_HOME.** ROK_HOME points to a per-test tempdir so
   code reading ``~/.rok/*`` via ``get_rok_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".rok"`` instead of the canonical
   ``get_rok_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ROK_SESSION_* inheritance** — the agent's current gateway
...
# ROK_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ROK_BEHAVIORAL_VARS = frozenset({
    "ROK_YOLO_MODE",
    "ROK_INTERACTIVE",
    "ROK_QUIET",
    "ROK_TOOL_PROGRESS",
    ...
    "ROK_SESSION_CHAT_ID",
    "ROK_SESSION_CHAT_NAME",
    "ROK_SESSION_THREAD_ID",
    "ROK_SESSION_SOURCE",
    "ROK_SESSION_KEY",
    "ROK_GATEWAY_SESSION",
    "ROK_PLATFORM",
    ...
    "ROK_TIMEZONE",
    "ROK_REDACT_SECRETS",
    "ROK_BACKGROUND_NOTIFICATIONS",
    "ROK_EXEC_ASK",
    "ROK_HOME_MODE",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.rok/kanban.db instead of the per-test ROK_HOME.
    "ROK_KANBAN_DB",
    "ROK_KANBAN_BOARD",
    "ROK_KANBAN_WORKSPACES_ROOT",
    "ROK_KANBAN_LOGS_ROOT",
    "ROK_KANBAN_TASK",
    "ROK_KANBAN_WORKSPACE",
    "ROK_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ROK_HOME to per-test tempdirs so code that
    reads ``~/.rok/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ROK_* vars that could change test semantics.
    for name in _ROK_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ROK_HOME to a per-test tempdir. Code that reads
    #    ``~/.rok/*`` via ``get_rok_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.rok/*`` via
    #    ``Path.home() / ".rok"`` instead of ``get_rok_home()``
    #    is a bug to fix at the callsite.
    fake_rok_home = tmp_path / "rok_test"
    fake_rok_home.mkdir()
    (fake_rok_home / "sessions").mkdir()
    (fake_rok_home / "cron").mkdir()
    (fake_rok_home / "memories").mkdir()
    (fake_rok_home / "skills").mkdir()
    monkeypatch.setenv("ROK_HOME", str(fake_rok_home))
    ...
    #    ~/.rok/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import rok_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_rok_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "rok_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal rok config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``rok-gateway`` process
...
# correlated with a ``tests/rok_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> rok-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart rok-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting rok/python patterns are also blocked.
    """
    ...
    def _matches_rok_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ROK_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_rok_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch rok-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m rok_cli.main".
                if (
                    "rok" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live rok-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting rok/python could hit the live gateway. "
