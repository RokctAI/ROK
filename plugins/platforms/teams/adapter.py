"""
Microsoft Teams platform adapter for Rok Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Rok"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        rok_action = data.get("rok_action", "")
        session_key = data.get("session_key", "")

        if not rok_action or not session_key:
        ...
        choice = choice_map.get(rok_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="rok_approve",
                    data={**btn_data_base, "rok_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="rok_approve",
                    data={**btn_data_base, "rok_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="rok_approve",
                    data={**btn_data_base, "rok_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="rok_approve",
                    data={**btn_data_base, "rok_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Rok\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.rok/.env")
