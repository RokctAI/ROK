"""SimpleX Chat platform adapter (Rok plugin).
...
This adapter ships as a Rok platform plugin under
``plugins/platforms/simplex/``. The Rok plugin loader scans the
...
discoverable and `rok setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Rok — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"rok-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``rok setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.rok/.env`` via ``rok_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("rok_cli.config not available; set SIMPLEX_* vars manually in ~/.rok/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `rok gateway status` without
