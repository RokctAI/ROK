...
# ``model_tools`` import time, so ``rok status``, ``rok chat``, etc.
...
# ``rok_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Rok is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a rok MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ROK_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _rok_home = _get_rok_home()
        except (ModuleNotFoundError, ImportError):
            _rok_home = _Path.home() / ".rok"
        self._thread_count_store = _ThreadCountStore(
            _rok_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Rok is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ROK_HOME", str(_Path.home() / ".rok"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ROK_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a rok MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Rok is thinking…' marker message.
        ...
        "Rok is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Rok is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Rok is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Rok is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Rok is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``rok setup``.

    The setup wizard at ``rok_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.rok/.env`` so the next
    gateway restart picks them up.
    """
    from rok_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. rok-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.rok/.env")
    print_info("Restart the gateway: rok gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``rok cron`` running as a
    separate process from ``rok gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Rok plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'rok-agent[google_chat]'",
        ...
        # the "Rok is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Rok is thinking…' marker message "
