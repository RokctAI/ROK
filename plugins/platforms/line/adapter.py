"""
LINE Messaging API platform adapter for Rok Agent.
...
to Rok Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ROK_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            rok_home = Path.home().joinpath(".rok").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``rok status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``rok status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``rok setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.rok/.env`` via ``rok_cli.config``.
    """
    print()
    ...
    try:
        from rok_cli.config import get_env_var, set_env_var
    except ImportError:
        print("rok_cli.config not available; set LINE_* vars manually in ~/.rok/.env")
        return
