# Rok Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: rok-achievements plugin
...
### `GET /api/plugins/rok-achievements/achievements`
...
### `POST /api/plugins/rok-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/rok-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/rok-achievements/overview`
...
- Persist plugin data under:
  - `~/.rok/plugins/rok-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.rok/plugins/rok-achievements/scan_checkpoint.json`
