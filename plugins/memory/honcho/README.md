...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/rok>
...
```bash
rok honcho setup    # full interactive wizard (cloud or local)
rok memory setup    # generic picker, also works
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ROK_HOME/honcho.json` | Profile-local (isolated Rok instances) |
| 2 | `~/.rok/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Rok profile: `rok` (default) or `rok.<profile>`.
...
| 4 | `per-session` strategy | Rok session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`rok-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`rok`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-rok-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening rok in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Rok session ID (timestamp + hex). Every `rok` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Rok profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`rok`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Rok profile: `rok` (default) or `rok.<profile>` (e.g. `rok -p coder` → host key `rok.coder`).
...
| `ROK_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `rok honcho setup` | Full interactive setup wizard |
| `rok honcho status` | Show resolved config for active profile |
| `rok honcho enable` / `disable` | Toggle Honcho for active profile |
| `rok honcho mode <mode>` | Change recall or observation mode |
| `rok honcho peer --user <name>` | Update user peer name |
| `rok honcho peer --ai <name>` | Update AI peer name |
| `rok honcho tokens --context <N>` | Set context token budget |
| `rok honcho tokens --dialectic <N>` | Set dialectic max chars |
| `rok honcho map <name>` | Map current directory to a session name |
| `rok honcho sync` | Create host blocks for all Rok profiles |
