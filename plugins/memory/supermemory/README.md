...
Config file: `$ROK_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `rok` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `rok-{identity}` → `rok-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Rok profile:

```json
{
  "container_tag": "rok-{identity}"
}
```

For a profile named `coder`, this resolves to `rok-coder`. The default profile resolves to `rok-default`. Without `{identity}`, all profiles share the same container.
