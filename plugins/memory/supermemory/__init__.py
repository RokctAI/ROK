...
def _load_supermemory_config(rok_home: str) -> dict:
    config = _default_config()
    config_path = Path(rok_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, rok_home: str) -> None:
    config_path = Path(rok_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `rok memory setup`.
        # All other options are documented for $ROK_HOME/supermemory.json
    ...
    def save_config(self, values, rok_home):
        ...
        _save_supermemory_config(sanitized, rok_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from rok_constants import get_rok_home
        self._rok_home = kwargs.get("rok_home") or str(get_rok_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._rok_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "rok", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "rok_memory", "target": target, "type": "explicit_memory"},
