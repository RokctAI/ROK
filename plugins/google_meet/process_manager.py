...
session-scoped state file under ``$ROK_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ROK_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_rok_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this rok install, leave it first —
    ...
    env = os.environ.copy()
    env["ROK_MEET_URL"] = url
    env["ROK_MEET_OUT_DIR"] = str(out)
    env["ROK_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ROK_MEET_HEADED"] = "1"
    if auth_state:
        env["ROK_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ROK_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ROK_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ROK_MEET_MODE"] = mode
    if realtime_model:
        env["ROK_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ROK_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ROK_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ROK_MEET_REALTIME_KEY"] = realtime_api_key
