...
Let the rok agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where rok runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `rok meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `rok meet` CLI |
...
| `cli.py` | `rok meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ROK_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `rok meet node {run,list,approve,remove,status,ping}` |
...
rok meet install                                      # pip + Chromium
rok meet setup                                        # preflight
rok meet auth                                         # optional
rok meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
rok meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.rok/.env
rok meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
rok meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
rok meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.rok/.env
rok meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, rok will **not** switch your system audio input automatically — the
...
rok plugins enable google_meet
rok meet node run --display-name my-mac --host 0.0.0.0 --port 18789
