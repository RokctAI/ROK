...
rok meet install                 # pip deps + Chromium (transcribe only)
rok meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
rok meet auth                    # optional; skips guest-lobby wait
rok meet setup                   # preflight checks
```

`rok meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or ROK_MEET_REALTIME_KEY in ~/.rok/.env
```
...
pip install playwright websockets && python -m playwright install chromium
rok plugins enable google_meet
rok meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
rok meet node approve my-mac ws://<mac-ip>:18789 <token>
rok meet node ping my-mac                   # confirm reachable
```

Run `rok meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Rok agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `rok meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `ROK_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `rok_meet_src`). |
...
Local:
```
$ROK_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$ROK_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `rok meet node approve`.
