...
Auto-tracks and cleans up ephemeral files created during Rok Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ROK_HOME` and `/tmp/rok-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ROK_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ROK_HOME` or `/tmp/rok-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ROK_HOME/disk-cleanup/` is itself excluded
- `$ROK_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
