"""disk_cleanup — ephemeral file cleanup for Rok Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ROK_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ROK_HOME and /tmp/rok-*
Never touches: ~/.rok/logs/ or any system directory.
"""
...
try:
    ...
    def get_rok_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ROK_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".rok").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ROK_HOME/logs/``."""
    return get_rok_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ROK_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ROK_HOME or ``/tmp/rok-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    rok_home = get_rok_home()
    try:
        path.resolve().relative_to(rok_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/rok-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("rok-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ROK_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ROK_HOME (but leave ROK_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "rok-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(rok_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == rok_home:
                continue
            try:
                rel_parts = dirpath.relative_to(rok_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    rok_home = get_rok_home()
    try:
        rel = path.resolve().relative_to(rok_home)
        ...
        # Path isn't under ROK_HOME (e.g. /tmp/rok-*) — fall through.
        pass
