/**
 * Rok Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.rok/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ROK_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__ROK_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "rok-kanban-dot-triage",
    todo: "rok-kanban-dot-todo",
    ready: "rok-kanban-dot-ready",
    running: "rok-kanban-dot-running",
    blocked: "rok-kanban-dot-blocked",
    done: "rok-kanban-dot-done",
    archived: "rok-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-rok-task";
  ...
  const DOCS_URL = "https://rok.rokct.ai/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://rok.rokct.ai/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "rok.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="rok-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "rok-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "rok-kanban-md",
  ...
  // standard `drop` event and our `rok-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("rok-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (col !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("rok-kanban-column--drop");
          if (col) col.classList.add("rok-kanban-column--drop");
          lastTarget = col;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("rok-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          lastTarget.dispatchEvent(new CustomEvent("rok-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        const token = window.__ROK_SESSION_TOKEN__ || "";
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "rok-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "rok-kanban-attention",
        "rok-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "rok-kanban-attention-bar" },
        h("span", { className: "rok-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "rok-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "rok-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "rok-kanban-attention-row",
                  "rok-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "rok-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "rok-kanban-attention-row-id" }, task.id),
                h("span", { className: "rok-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "rok-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "rok-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "rok-kanban-diag-action-btn",
      action.suggested ? "rok-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " rok-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".rok-kanban-drawer-comment-row input, .rok-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "rok-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("rok-kanban-diag", sevClass) },
      h("div", { className: "rok-kanban-diag-header" },
        h("span", { className: "rok-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "rok-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "rok-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "rok-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "rok-kanban-diag-data-row" },
                  h("span", { className: "rok-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "rok-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "rok-kanban-diag-data-row" },
                h("span", { className: "rok-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "rok-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "rok-kanban-diag-reassign-row" },
            h("span", { className: "rok-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "rok-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "rok-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "rok-kanban-diag-msg",
              msg.ok ? "rok-kanban-diag-msg--ok" : "rok-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "rok-kanban-section" },
      h("div", { className: "rok-kanban-section-head-row" },
        h("span", { className: "rok-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "rok-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "rok-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "rok-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "rok-kanban-docs-link",
      title: "Open Rok Kanban docs in a new tab",
      "aria-label": "Rok Kanban documentation",
    }, "?");
  }
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "rok-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "rok-kanban-boardswitcher" },
      h("div", { className: "rok-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "rok-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "rok-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "rok-kanban-dialog-title" },
        ...
        h("div", { className: "rok-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Rok profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "rok-kanban-bulk" },
      h("span", { className: "rok-kanban-bulk-count" },
      ...
      h("div", { className: "rok-kanban-bulk-priority",
      ...
      h("div", { className: "rok-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Rok profile. Pick a profile (or unassign) and click Apply." },
        h(Select, {
          value: assignee,
      ...
      h("label", { className: "rok-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h("input", {
          type: "checkbox",
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".rok-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "rok-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("rok-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("rok-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "rok-kanban-column",
        dragOver ? "rok-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "rok-kanban-column-header",
                 title: colHelp || "" },
        h("input", {
          type: "checkbox",
          className: "rok-kanban-col-check",
        ...
        h("span", { className: cn("rok-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "rok-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "rok-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "rok-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "rok-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "rok-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "rok-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "rok-kanban-lane" },
                  h("div", { className: "rok-kanban-lane-head" },
                    h("span", { className: "rok-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "rok-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "rok-kanban-card--stale-red";
    if (age >= tier.amber) return "rok-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".rok-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "rok-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "rok-kanban-card",
        props.selected ? "rok-kanban-card--selected" : "",
        props.failed ? "rok-kanban-card--failed" : "",
        props.draggingSource ? "rok-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "rok-kanban-card-content" },
          h("div", { className: "rok-kanban-card-row" },
            h("label", {
              className: "rok-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h("input", {
                type: "checkbox",
                className: "rok-kanban-card-check",
            ...
            h("span", { className: "rok-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or rok kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "rok-kanban-warning-badge",
                    "rok-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            progress
              ? h("span", {
                  className: cn(
                    "rok-kanban-progress",
                    progress.done === progress.total ? "rok-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "rok-kanban-needs-assignee",
          ...
          h("div", { className: "rok-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "rok-kanban-card-row rok-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "rok-kanban-assignee",
                            title: `Assigned to Rok profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "rok-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "rok-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "rok-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "rok-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Rok profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Rok profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "rok-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "rok-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "rok-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "rok-kanban-drawer-close",
        ...
        data ? h("div", { className: "rok-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "rok-kanban-drawer-body" },
      h("div", { className: "rok-kanban-drawer-title" },
        h("span", { className: cn("rok-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
      ...
      t.result ? h("div", { className: "rok-kanban-section" },
        h("div", { className: "rok-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "rok-kanban-section" },
        h("div", { className: "rok-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "rok-kanban-comment" },
                h("div", { className: "rok-kanban-comment-head" },
                  h("span", { className: "rok-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "rok-kanban-comment-ago" },
      ...
      h("div", { className: "rok-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "rok-kanban-event",
              isDiag ? "rok-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "rok-kanban-event-header" },
                  h("span", { className: "rok-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "rok-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "rok-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "rok-kanban-event-header-plain" },
                  h("span", { className: "rok-kanban-event-kind" }, e.kind),
                  h("span", { className: "rok-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "rok-kanban-event-phantom-row" },
                  h("span", { className: "rok-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "rok-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "rok-kanban-event-payload" },
    ...
    return h("div", { className: "rok-kanban-section" },
      h("div", { className: "rok-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              ...
              className: "rok-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `rok-kanban-run--${r.outcome || r.status || "ended"}`
          : "rok-kanban-run--active";
        return h("div", { key: r.id, className: cn("rok-kanban-run", outcomeClass) },
          h("div", { className: "rok-kanban-run-head" },
            h("span", { className: "rok-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "rok-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "rok-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "rok-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "rok-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "rok-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "rok-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "rok-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "rok-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "rok-kanban-pre rok-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "rok-kanban-meta-row" },
      h("span", { className: "rok-kanban-meta-label" }, props.label),
      h("span", { className: "rok-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "rok-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "rok-kanban-meta-row" },
        h("span", { className: "rok-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "rok-kanban-meta-value rok-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "rok-kanban-meta-row" },
        h("span", { className: "rok-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "rok-kanban-meta-value rok-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "rok-kanban-section" },
      h("div", { className: "rok-kanban-section-head-row" },
        h("span", { className: "rok-kanban-section-head" }, tx(t, "description", "Description")),
        editing
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "rok-kanban-section" },
      h("div", { className: "rok-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "rok-kanban-deps-row" },
        h("span", { className: "rok-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "rok-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "rok-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "rok-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "rok-kanban-dep-chip-x",
      ...
      h("div", { className: "rok-kanban-deps-row" },
        h("span", { className: "rok-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "rok-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "rok-kanban-actions" },
        specifyButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "rok-kanban-section" },
      ...
      h("div", { className: "rok-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "rok-kanban-home-sub rok-kanban-home-sub--on"
              : "rok-kanban-home-sub",
          }, label);
        })
      )
  ...
  if (window.__ROK_PLUGINS__ && typeof window.__ROK_PLUGINS__.register === "function") {
    window.__ROK_PLUGINS__.register("kanban", KanbanPage);
  }
})();
