...
``_check_spotify_available()`` — when the user has not run ``rok auth
spotify``, the tools remain registered (so they appear in ``rok
...
The Spotify auth flow (``rok auth spotify``), CLI plumbing, and docs
