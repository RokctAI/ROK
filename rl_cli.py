...
if tinker_atropos_dir.exists():
    ...
    os.environ['ROK_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to rok-agent directory if submodule not found
...
from rok_constants import get_rok_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_rok_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.rok/config.yaml if not provided)
    ...
    """
    # Load config from ~/.rok/config.yaml
    config = load_rok_config()
