...
_ROK_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (ROK_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `rok tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ROK_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Rok toolsets (CLI + messaging platforms)
    #
    ...
    "rok-cli": {
        ...
        "tools": _ROK_CORE_TOOLS,
        "includes": []
    },

    "rok-cron": {
        # Mirrors rok-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `rok tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as rok-cli; gated by `rok tools`",
        "tools": _ROK_CORE_TOOLS,
        "includes": []
    },
    ...
    "rok-email": {
        "description": "Email bot toolset - interact with Rok via email (IMAP/SMTP)",
        "tools": _ROK_CORE_TOOLS,
        "includes": []
    },
    ...
    "rok-sms": {
        "description": "SMS bot toolset - interact with Rok via SMS (Twilio)",
        "tools": _ROK_CORE_TOOLS,
        "includes": []
    },
    ...
    "rok-gateway": {
        ...
        "tools": [],
        "includes": ["rok-telegram", "rok-discord", "rok-whatsapp", "rok-slack", "rok-signal", "rok-bluebubbles", "rok-homeassistant", "rok-email", "rok-sms", "rok-mattermost", "rok-matrix", "rok-dingtalk", "rok-feishu", "rok-wecom", "rok-wecom-callback", "rok-weixin", "rok-qqbot", "rok-webhook", "rok-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (rok-<name>).
        # Gives them _ROK_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("rok-"):
            platform_name = name[len("rok-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ROK_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
