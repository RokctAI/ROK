# OpenAI-Compatible API Server for Rok Agent
...
SSE streaming. By exposing this endpoint, rok-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  rok-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.rok/.env`
2. Run `rok gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with rok-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns rok-agent as a model) |
...
Authorization: Bearer rok-api-key-here
...
But rok-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "rok-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured rok-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of rok-agent's core prompt
...
Once implemented, rok-agent works as a drop-in backend for:
