...
SSE streaming. By exposing this endpoint, rok becomes instantly
...
4. Chat with rok through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns rok as a model) |
...
But rok sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "rok"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured rok model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of rok's core prompt
...
Once implemented, rok works as a drop-in backend for:
