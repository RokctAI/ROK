"""rok hooks — inspect and manage shell-script hooks.

Usage::

    rok hooks list
    rok hooks test <event> [--for-tool X] [--payload-file F]
    rok hooks revoke <command>
    rok hooks doctor

Consent records live under ``~/.rok/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.rok/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``rok hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: rok hooks {list|test|revoke|doctor}")
        print("Run 'rok hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.rok/config.yaml.")
        print("See `rok hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `rok hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `rok hooks test` and `rok hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Rok wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `rok hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `rok hooks doctor` would execute
