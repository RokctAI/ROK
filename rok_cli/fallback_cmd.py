"""
rok fallback — manage the fallback provider chain.
...
Subcommands:
  rok fallback [list]   Show the current fallback chain (default when no subcommand)
  rok fallback add      Pick provider + model via the same picker as `rok model`,
                           then append the selection to the chain
  rok fallback remove   Pick an entry to delete from the chain
  rok fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.rok/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from rok_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `rok model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  rok fallback add")
        print()
        return
    ...
    print("  Docs: https://rok.rokct.ai/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `rok model`, then append the selection to the chain."""
    from rok_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `rok model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `rok fallback list` to view, or `rok fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``rok fallback [subcommand]``."""
