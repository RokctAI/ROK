...
def _write_codex_cli_tokens(
    ...
    *,
    ...
    When Rok refreshes a token it consumes the old refresh_token; if we
...
def resolve_codex_runtime_credentials(
    *,
    ...
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to Rok auth store")
            print("⚠️  Migrating Codex credentials to Rok's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `rok auth` to create a fully independent session.\n")
    ...
    *,
    ...
    if effective_ca:
        ...
        if not os.path.isfile(ca_path):
            import logging
            logging.getLogger("rok.auth").warning(
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `rok status` output."""
