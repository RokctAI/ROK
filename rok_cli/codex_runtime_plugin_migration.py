"""Migrate Rok' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Rok' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Rok mcp_servers.<n>.command/args/env  → codex stdio transport
  Rok mcp_servers.<n>.url/headers       → codex streamable_http transport
  Rok mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Rok mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Rok-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Rok' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Rok config.")
...
# Rok keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Rok' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, rok_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Rok MCP server config to the codex inline-table dict
    ...
    if not isinstance(rok_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(rok_cfg.get("command"))
    has_url = bool(rok_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(rok_cfg["command"])
        args = rok_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = rok_cfg.get("env") or {}
        if env:
        ...
        out["url"] = str(rok_cfg["url"])
        headers = rok_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # Rok' transport: sse hint is informational; codex auto-negotiates
        if rok_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in rok_cfg:
        try:
            out["tool_timeout_sec"] = float(rok_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in rok_cfg:
        try:
            out["startup_timeout_sec"] = float(rok_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if rok_cfg.get("enabled") is False:
        out["enabled"] = False
    ...
    for key in rok_cfg:
        ...
        elif key not in _KNOWN_ROK_KEYS:
            skipped.append(f"{key} (unknown Rok key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ROK_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Rok)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Rok' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Rok' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Rok
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="rok-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ROK_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed rok-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.rok`` symlink that happens to live under ``/private/var/folders``
...
def _build_rok_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Rok' own
    ...
    so a rok installed under /opt/, /usr/local/, or a venv all work.
    ROK_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ROK_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_rok_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ROK_HOME its launcher sets
    ...
    # — that would override the launcher's ROK_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ROK_HOME", tmp_path) would
    ...
    rok_home = os.environ.get("ROK_HOME") or ""
    if rok_home and _looks_like_test_tempdir(rok_home):
        rok_home = ""
    if rok_home:
        env["ROK_HOME"] = rok_home
    # PYTHONPATH passes through so a worktree-launched rok finds the
    ...
    env["ROK_QUIET"] = "1"
    env["ROK_REDACT_SECRETS"] = env.get("ROK_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_rok_tools: bool = True,
) -> MigrationReport:
    """Translate Rok mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        rok_config: full ~/.rok/config.yaml dict
    ...
    rok_servers = (rok_config or {}).get("mcp_servers") or {}
    if not isinstance(rok_servers, dict):
        report.errors.append(
            "mcp_servers in Rok config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in rok_servers.items():
    ...
    # Inject Rok' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Rok for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/rok_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_rok_tools:
        translated["rok-tools"] = _build_rok_tools_mcp_entry()
        if "rok-tools" not in report.migrated:
            report.migrated.append("rok-tools")
