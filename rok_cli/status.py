"""
Status command for rok CLI.

Shows the status of all Rok Agent components.
"""
...
from rok_cli.auth import AuthError, resolve_provider
from rok_cli.colors import Colors, color
from rok_cli.config import get_env_path, get_env_value, get_rok_home, load_config
from rok_cli.models import provider_label
from rok_cli.nous_subscription import get_nous_subscription_features
from rok_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``rok config``'s
...
def show_status(args):
    """Show status of all Rok Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Rok Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: rok auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: rok model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: rok auth add minimax-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: rok model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from rok_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'rok-agent[vercel]')"
    ...
    try:
        from rok_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_rok_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_rok_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'rok doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'rok setup' to configure", Colors.DIM))
    print()
