...
def show_status(args):
    ...
    try:
        from rok_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: rok model)'}"
    )
    if nous_logged_in:
    ...
    if not terminal_env:
        ...
        # (rok status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
