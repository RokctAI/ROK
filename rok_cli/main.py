...
from rok_constants import OPENROUTER_BASE_URL
...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Rok isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  rok setup")
        print()

        from rok_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
...
def select_provider_and_model(args=None):
    ...
    from rok_cli.config import get_compatible_custom_providers, load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("ROK_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from rok_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from rok_cli.config import get_env_value, save_env_value
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from rok_cli.config import get_env_value, save_config, save_env_value
    from rok_cli.nous_subscription import (
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set ROK_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from rok_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from rok_cli.config import get_env_value, save_env_value, load_config, save_config
    from rok_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(f"    Rok will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _update_via_zip(args):
    ...
    branch = "main"
    zip_url = f"https://github.com/RokctAI/rok/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="rok-update-")
        zip_path = os.path.join(tmp_dir, f"rok-{branch}.zip")
        ...
        # GitHub ZIPs extract to rok-<branch>/
        extracted = os.path.join(tmp_dir, f"rok-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("rok-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Rok couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Rok couldn't drop the saved stash entry.")
...
OFFICIAL_REPO_URLS = {
    "https://github.com/RokctAI/rok.git",
    "git@github.com:Rokctai/rok.git",
    "https://github.com/RokctAI/rok",
    "git@github.com:Rokctai/rok",
}
OFFICIAL_REPO_URL = "https://github.com/RokctAI/rok.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from Rokctai/rok.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/RokctAI/rok.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/RokctAI/rok.git' to add later.")
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "rok[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    ...
    try:
        ...
        try:
            from rok_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'rok config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
        ...
        # cgroup.  ``systemctl restart rok-gateway`` kills everything in the
        ...
        try:
            ...
            if supports_systemd_services():
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "rok-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. rok-gateway.service or rok-gateway-coder.service
            ...
            if is_macos():
                try:
                    from rok_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    if len(killed_pids) > 1:
                        print("    (or: rok -p <profile> gateway run  for each profile)")
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  rok profile alias {name} --name <custom>")
        ...
        name = args.profile_name
        from rok_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
...
def cmd_dashboard(args):
    ...
    try:
        ...
        print("Install them with:  pip install rok[web]")
        sys.exit(1)
...
def cmd_completion(args):
    ...
    from rok_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="rok",
        description="Rok Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    rok                        Start interactive chat
    rok chat -q "Hello"        Single query mode
    rok -c                     Resume the most recent session
    rok -c "my project"        Resume a session by name (latest in lineage)
    rok --resume <session_id>  Resume a specific session by ID
    rok setup                  Run setup wizard
    rok logout                 Clear stored authentication
    rok auth add <provider>    Add a pooled credential
    rok auth list              List pooled credentials
    rok auth remove <p> <t>    Remove pooled credential by index, id, or label
    rok auth reset <provider>  Clear exhaustion status for a provider
    rok model                  Select default model
    rok config                 View configuration
    rok config edit            Edit config in $EDITOR
    rok config set model gpt-4 Set a config value
    rok gateway                Run messaging gateway
    rok -s rok-dev,github-auth
    rok -w                     Start in isolated git worktree
    rok gateway install        Install gateway background service
    rok sessions list          List past sessions
    rok sessions browse        Interactive session picker
    rok sessions rename ID T   Rename/title a session
    rok logs                   View agent.log (last 50 lines)
    rok logs -f                Follow agent.log in real time
    rok logs errors            View errors.log
    rok logs --since 1h        Lines from the last hour
    rok debug share             Upload debug report for support
    rok update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Rok Agent"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: rok-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
...
""",
    ...
    backup_parser = subparsers.add_parser(
        ...
        description="Create a zip archive of your entire Rok configuration, "
                    "skills, sessions, and data (excludes the rok codebase). "
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            import shutil
            rok_bin = shutil.which("rok")
            if rok_bin:
                os.execvp(rok_bin, ["rok", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "rok_cli.main", "--resume", selected_id],
                )
