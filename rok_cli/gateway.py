"""
Gateway subcommand for rok CLI.

Handles: rok gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_rok_home is imported lazily at call sites to avoid ImportError
# when rok_constants is cached from a pre-update version during `rok update`.
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "rok-gateway*",
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "rok CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``rok gateway status`` from falsely counting the ``rok`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``rok gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "rok_cli.main gateway",
        "rok_cli.main --profile",
        "rok_cli.main -p",
        "rok_cli/main.py gateway",
        "rok_cli/main.py --profile",
        "rok_cli/main.py -p",
        "rok gateway",
        "gateway/run.py",
    ]
    current_home = str(get_rok_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"ROK_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. ROK_HOME
        ...
        if "--profile " in command or " -p " in command:
            return False
        if "ROK_HOME=" in command and f"ROK_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``rok update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Rok profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Rok profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "rok_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``rok update`` users
    # with no running gateway until they re-invoke ``rok gateway``
...
def _sync_rok_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ROK_HOME``.

    Under ``sudo``, ``ROK_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_rok_home` falls back to ``/root/.rok`` — the wrong
    profile. The unit file pins ``ROK_HOME`` for the actual gateway
    ...
    unit_home = env.get("ROK_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ROK_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["ROK_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}rok gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}rok gateway restart{scope_flag}")
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from rok_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `rok gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``rok gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from rok_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ROK_HOME-scoped).
...
from rok_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``rok gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``ROK_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("ROK_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "rok-gateway"
SERVICE_DESCRIPTION = "Rok Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ROK_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ROK_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from rok_constants import get_default_rok_root
    home = get_rok_home().resolve()
    default = get_default_rok_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ROK_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(rok_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when ROK_HOME is a named profile.

    For ``~/.rok/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        rok_home: Optional explicit ROK_HOME path. Defaults to the current
            ``get_rok_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from rok_constants import get_default_rok_root
    home = Path(rok_home or str(get_rok_home())).resolve()
    default = get_default_rok_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this ROK_HOME.

    Default ``~/.rok`` returns ``rok-gateway`` (backward compatible).
    Profile ``~/.rok/profiles/coder`` returns ``rok-gateway-coder``.
    Any other ROK_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Rok installs that predate the
# rok-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (rok-gateway-*.service) and unrelated third-party
# "rok" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("rok.service",)
...
def _find_legacy_rok_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Rok gateway units.

    Detects unit files installed by older Rok versions that used a
    different service name (e.g. ``rok.service`` before the rename to
    ``rok-gateway.service``). When both a legacy unit and the current
    ``rok-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``rok-gateway-coder.service`` and unrelated third-party
      ``rok-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``rok.service`` running an unrelated
...
def has_legacy_rok_units() -> bool:
    """Return True when any legacy Rok gateway unit files exist."""
    return bool(_find_legacy_rok_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Rok gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_rok_units()
    if not legacy:
        return
    print_warning("Legacy Rok gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current rok-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    rok gateway migrate-legacy")


def remove_legacy_rok_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Rok gateway unit files.

    Iterates over whatever ``_find_legacy_rok_units()`` returns — which is
    ...
    legacy = _find_legacy_rok_units()
    if not legacy:
        print("No legacy Rok gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Rok gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: rok gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo rok gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    rok gateway uninstall")
    print_info("    sudo rok gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Rok can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo rok gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo rok gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo rok gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Rok often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.rok`` → ``ai.rok.gateway.plist`` (backward compatible).
    Profile ``~/.rok/profiles/coder`` → ``ai.rok.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.rok.gateway-{suffix}" if suffix else "ai.rok.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.rok/rok-agent  -> /home/alice/.rok/rok-agent
      /opt/rok                 -> /opt/rok  (kept as-is)
...
def _rok_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ROK_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_rok_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.rok                    → /home/alice/.rok
      /root/.rok/profiles/coder     → /home/alice/.rok/profiles/coder
      /opt/custom-rok               → /opt/custom-rok  (kept as-is)
    """
    current_rok = get_rok_home().resolve()
    current_default = (Path.home() / ".rok").resolve()
    target_default = Path(target_home_dir) / ".rok"

    # Default ~/.rok → remap to target user's default
    if current_rok == current_default:
        return str(target_default)

    # Profile or subdir of ~/.rok → preserve the relative structure
    try:
        relative = current_rok.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.rok) — keep as-is
        return str(current_rok)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    rok_node = rok_home / "node" / "bin"
    if rok_node.is_dir():
        candidates.append(str(rok_node))
    rok_nm = rok_home / "node_modules" / ".bin"
    if rok_nm.is_dir():
        candidates.append(str(rok_nm))
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        rok_home = _rok_home_for_target_user(home_dir)
        profile_arg = _profile_arg(rok_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m rok_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
"""
    ...
    rok_home = str(get_rok_home().resolve())
    profile_arg = _profile_arg(rok_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only ROK_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ROK_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../rok_test`` path into
    # ``Environment="ROK_HOME=..."``. Writing that to the developer's
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Rok install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo rok gateway uninstall --system")
    print_info("         rok gateway install")
    print_info("         rok gateway start")
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (rok.service from pre-rename installs)
    # before installing the new rok-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}rok gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}rok gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}rok gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_rok_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `rok gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `rok gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}rok gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}rok gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}rok gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}rok gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.rok.gateway-{suffix}" if suffix else "ai.rok.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Rok install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  rok gateway status             # Check status")
    from rok_constants import display_rok_home as _dhh
...
def launchd_stop():
    ...
    # `rok gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Rok install")
    else:
        print("⚠ Service definition is stale relative to the current Rok install")
        print("  Run: rok gateway start")
    ...
    if deep:
        log_file = get_rok_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("ROK_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Rok gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'rok' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/rok/docker/entrypoint.sh before the Rok command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$ROK_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set ROK_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `rok gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set ROK_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `rok gateway restart` which already
    ...
    # `rok gateway start/restart` — leaving the gateway vulnerable to
    ...
    # ROK_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("ROK_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from rok_constants import get_rok_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Rok will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @rok:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. rok) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Rok delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Rok agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Rok will use (e.g., rok@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Rok connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: rok pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Rok will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``rok setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run rok under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.rok/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_rok_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'rok pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: rok pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'rok gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from rok_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from rok_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Rok will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Rok will store the returned account_id/token in ~/.rok/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `rok gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_rok_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `rok pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `rok pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"Rokagent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``rok_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from rok_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.rok/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.rok/.env: {', '.join(required)}")
    else:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: rok gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: rok gateway install")
                else:
                    print_info("  You can install later: rok gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo rok gateway install --system")
                    print_info("  Or run in foreground:  rok gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: rok gateway run")
                print_info("  For persistence:   tmux new -s rok 'rok gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from rok_constants import display_rok_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: rok gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup rok gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: rok gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'rok gateway setup' when ready.")
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``rok gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: rok gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: rok gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s rok 'rok gateway run'")
                print()
            ...
            print()
            print("  rok gateway run                              # direct foreground")
            print("  tmux new -s rok 'rok gateway run'         # persistent via tmux")
            print("  nohup rok gateway run > ~/.rok/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: rok gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: rok gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: rok gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: rok gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: rok gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  rok gateway install")
                else:
                    print("To install as a service:")
                    print("  rok gateway install")
                    print("  sudo rok gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  rok gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup rok gateway run > ~/.rok/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s rok 'rok gateway run'         # persistent via tmux")
                    print("  nohup rok gateway run > ~/.rok/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  rok gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  rok gateway install  # Install as user service")
                    print("  sudo rok gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Rok gateway unit files from
        # pre-rename installs (e.g. rok.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_rok_units(interactive=not yes, dry_run=dry_run)
