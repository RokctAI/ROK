...
on Japanese locales, etc.).  Rok's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``rok_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Rok is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ROK_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ROK_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Rok-managed Git install directories so
    ...
    # already-running shells don't see the change — which means rok
    ...
    # install without Rok-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Rok's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Rok's
    ``rok config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Rok (User env var in Windows Settings, or
    export in a PowerShell profile) and Rok picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Rok-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\rok\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``rok`` from right after install) retain their old PATH.

    Any subprocess Rok spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Rok-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "rok", "git", "cmd"),
        os.path.join(local_appdata, "rok", "git", "bin"),
        os.path.join(local_appdata, "rok", "git", "usr", "bin"),
        # Rok venv Scripts directory — host of the rok.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes rok, but harmless to include.
        os.path.join(local_appdata, "rok", "rok-agent", "venv", "Scripts"),
