"""
Single source of truth for provider identity in Rok Agent.
...
2. **Rok overlays** — transport type, auth patterns, aggregator flags,
...
# -- Rok overlay ----------------------------------------------------------
# Rok-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class RokOverlay:
    """Rok-specific provider metadata layered on top of models.dev."""
...
ROK_OVERLAYS: Dict[str, RokOverlay] = {
    "openrouter": RokOverlay(
    ...
    "nous": RokOverlay(
        ...
        base_url_override="https://inference-api.rokctai.com/v1",
    ),
    "openai-codex": RokOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "xai-oauth": RokOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": RokOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ROK_QWEN_BASE_URL",
    ),
    "google-gemini-cli": RokOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
    ...
    "copilot-acp": RokOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": RokOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": RokOverlay(
    ...
    "alibaba-coding-plan": RokOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "rok", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Rok overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Rok overlay
    ...
    overlay = ROK_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + rok extra
    ...
    if overlay is not None:
        # Rok-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in ROK_OVERLAYS
    if provider == "bedrock":
