...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_rok_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:rok_pkce",
    ...
    raise SystemExit(f"`rok auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # If this was a singleton-seeded credential (OAuth device_code, rok_pkce),
    ...
    # re-seeded on the next load_pool() call.
    ...
    elif removed.source == "rok_pkce" and provider == "anthropic":
        from rok_constants import get_rok_home
        oauth_file = get_rok_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared Rok Anthropic OAuth credentials")
        ...
        print("Run `rok auth add anthropic` to re-enable if needed.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `rok auth` is called bare."""
