...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # <rok-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `rok --profile <name>
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `rok auth
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Rok reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `rok auth status spotify`.")
