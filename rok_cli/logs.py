"""``rok logs`` — view and filter Rok log files.
...
Usage examples::
    ...
    rok logs                    # last 50 lines of agent.log
    rok logs -f                 # follow agent.log in real time
    rok logs errors             # last 50 lines of errors.log
    rok logs gateway -n 100    # last 100 lines of gateway.log
    rok logs --level WARNING    # only WARNING+ lines
    rok logs --session abc123   # filter by session ID substring
    rok logs --component tools  # only tool-related lines
    rok logs --since 1h         # lines from the last hour
    rok logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_rok_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Rok runs — try 'rok chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_rok_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_rok_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_rok_home()}/logs/")
        return

    print(f"Log files in {display_rok_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'rok chat' to generate logs)")
