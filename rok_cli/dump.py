"""
Dump command for rok CLI.

Outputs a compact, plain-text summary of the user's Rok setup
...
from rok_cli.config import get_rok_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``rok dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from rok_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(rok_home: Path) -> int:
...
def _cron_summary(rok_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = rok_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"rok_home:      {display_rok_home()}")
    ...
    toolsets = config.get("toolsets", ["rok-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(rok_home)}")
    lines.append(f"  skills:             {_count_skills(rok_home)}")
