...
Reads the user's Nous OAuth state from ``~/.rok/auth.json``, refreshes
...
``inference-api.rokctai.com`` accepts as a bearer. The refresh helper
already handles both — see :func:`rok_cli.auth.refresh_nous_oauth_from_state`.
"""
...
# Endpoints inference-api.rokctai.com actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def get_credential(self) -> UpstreamCredential:
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `rok login nous` first."
                )
            ...
            if not agent_key:
                raise RuntimeError(
                    ...
                    "Try `rok login nous` to re-authenticate."
                )
    ...
    # to rok_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
