"""
Configuration management for Rok Agent.

Config files are stored in ~/.rok/ for easy access:
- ~/.rok/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.rok/.env         - API keys and secrets

This module provides:
- rok config          - Show current configuration
- rok config edit     - Open config in editor
- rok config set      - Set a specific value
- rok config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.rok/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``rok logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  rok config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `rok plugins enable
    # observability/langfuse` or `rok tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ROK_LANGFUSE_ENV",
    "ROK_LANGFUSE_RELEASE",
    "ROK_LANGFUSE_SAMPLE_RATE",
    "ROK_LANGFUSE_MAX_CHARS",
    "ROK_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from rok_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ROK_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_rok_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Rok is running in package-manager-managed mode.

    Two signals: the ROK_MANAGED env var (set by the systemd service),
    or a .managed marker file in ROK_HOME (set by the NixOS activation
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Rok was installed: 'nixos', 'homebrew', 'git', or 'pip'."""
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        ...
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade rok-agent"
        return "pip install --upgrade rok-agent"
    return "rok update"
...
def format_managed_message(action: str = "modify this Rok installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ROK_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Rok installation is managed by NixOS "
            f"(ROK_MANAGED={env_hint}).\n"
            "Edit services.rok-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Rok installation is managed by Homebrew "
            f"(ROK_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade rok-agent"
        )

    return (
        f"Cannot {action}: this Rok installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Rok."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ROK_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, rok_bin
    or None if container mode is not active, we're already inside the
    container, or ROK_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ROK_DEV") == "1":
        return None
    ...
    container_mode_file = get_rok_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "rok-agent")
    exec_user = info.get("exec_user", "rok")
    rok_bin = info.get("rok_bin", "/data/current-package/bin/rok")
...
# Re-export from rok_constants — canonical definition lives there.
from rok_constants import get_rok_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_rok_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_rok_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the rok group can
    share state with the gateway service.

    The mode can be overridden via the ROK_HOME_MODE environment variable
    (e.g. ROK_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ROK_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ROK_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Rok runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ROK_CONTAINER") or os.environ.get("ROK_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ROK_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ROK_HOME if the user doesn't have one yet."""
...
def ensure_rok_home():
    """Ensure ~/.rok directory structure exists with secure permissions.
...
def _ensure_rok_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ROK_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Rok-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Rok auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Rok sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Rok
        ...
        # bundled Rok image, which drops to the `rok` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Rok sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Rok should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``rok chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.rok/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: rok sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Rok truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``rok kanban specify`` (single id or --all). Set a
        ...
        # model; override via `rok model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `rok --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `rok -c` muscle memory.  Default off so existing
        # users aren't surprised.  ROK_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/rok`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.rok/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.rok/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Rok feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Rok auto-pauses the goal and
    ...
    # always goes to ~/.rok/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ROK_SKILL_DIR} and ${ROK_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `rok curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.rok/skills/ into
        # ~/.rok/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `rok curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for rok-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Rok Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ROK_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.rok/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ROK_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `rok doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``rok_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Rok to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via ROK_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `rok -p <assignee> chat -q ...` for
    ...
    "code_execution": {
        ...
        #     rok-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.rok/logs/.
    ...
    # update model picker lists without shipping a rok-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://rok.rokct.ai/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `rok model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.rok/state.db.
    ...
    # ``rok update`` behaviour.
    "updates": {
        # Run a full ``rok backup``-style zip of ROK_HOME before every
        # ``rok update``.  Backups land in ``<ROK_HOME>/backups/`` and
        # can be restored with ``rok import <path>``.  Off by default —
        # on large ROK_HOME directories the zip can add minutes to every
    ...
    # `rok tools`. These settings tune the backing Responses API call.
    "x_search": {
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'rok model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. rokctai.com -> firecrawl-gateway.rokctai.com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Rok auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @rok:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ROK_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #rok)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: rok-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'rok-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Rok API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Rok API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Rok API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ROK_TOOL_PROGRESS and ROK_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.rok.config`` entries, then checks
    ...
    try:
        ...
        # should never break `rok update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Rok versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``rok_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'rok setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Rok won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'rok doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ROK_HOME", "~/.rok")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ROK_TOOL_PROGRESS")
            old_mode = get_env_value("ROK_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ROK_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ROK_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ROK_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ROK_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ROK_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_rok_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `rok plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `rok curator status` has no
    ...
    #   3. Creates `~/.rok/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_rok_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_rok_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: rok config set <key> <value>")
    ...
    # metadata.rok.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _rok_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.rok/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.rok/config.yaml.
    ...
    (which change ``ROK_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_rok_home()
...
#   openai-codex (OAuth — rok auth) — OpenAI Codex
#   nous         (OAuth — rok auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.rok/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.rok/.env.
    ...
    interactive menu render (`rok tools`, `rok setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `rok tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Rok env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.rok/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.rok/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.rok/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.rok/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Rok — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Rok vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.rok/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Rok Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  rok config edit     # Edit config file", Colors.DIM))
    print(color("  rok config set <key> <value>", Colors.DIM))
    print(color("  rok setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: rok config set <key> <value>")
            print()
            print("Examples:")
            print("  rok config set model anthropic/claude-sonnet-4")
            print("  rok config set terminal.backend docker")
            print("  rok config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'rok config migrate' to add them")
        ...
        print("  rok config           Show current configuration")
        print("  rok config edit      Open config in editor")
        print("  rok config set <key> <value>   Set a config value")
        print("  rok config check     Check for missing/outdated config")
        print("  rok config migrate   Update config with new options")
        print("  rok config path      Show config file path")
        print("  rok config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``rok config`` UI so users
