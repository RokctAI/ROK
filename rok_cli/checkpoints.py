"""`rok checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.rok/checkpoints/``.  Actions:

    rok checkpoints               # same as `status`
    rok checkpoints status        # total size, project count, breakdown
    rok checkpoints list          # per-project checkpoint counts + workdir
    rok checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    rok checkpoints clear [-f]    # nuke the entire base (asks first)
    rok checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    rok checkpoints
    rok checkpoints prune --retention-days 3 --max-size-mb 200
    rok checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: rok checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``rok checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `rok checkpoints` → status
