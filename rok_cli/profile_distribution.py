"""Profile distributions — shareable, packaged Rok profiles via git.

A distribution is a Rok profile published as a git repository (or
...
* ``rok profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``rok skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``rok profile``, not a parallel tree):

    rok profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "rok-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            rok_requires=str(data.get("rok_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.rok_requires:
            out["rok_requires"] = self.rok_requires
        if self.author:
...
def check_rok_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Rok {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``rok profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Rok profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Rok distribution."
        )

    # Version check up-front so we fail fast
    check_rok_requires(manifest.rok_requires, rok_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.rok).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="rok_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `rok profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`rok profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="rok_dist_update_") as tmp:
        plan = plan_install(
