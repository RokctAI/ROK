"""
Profile management for multiple isolated Rok instances.

Each profile is a fully independent ROK_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.rok/profiles/<name>/`` by default.

The "default" profile is ``~/.rok`` itself — backward compatible,
zero migration needed.

Usage::

    rok profile create coder          # fresh profile + bundled skills
    rok profile create coder --clone  # also copy config, .env, SOUL.md, skills
    rok profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    rok -p coder chat                 # or via flag
    rok profile use coder             # set as sticky default
    rok profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See rok_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `rok profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `rok
...
# `rok skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Rok infrastructure directories that only the default profile
         (``~/.rok``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_rok_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.rok) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "rok-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "rok", "default", "test", "tmp", "root", "sudo",
})

# Rok subcommands that cannot be used as profile names/aliases
_ROK_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the rok root, NOT to the current ROK_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ROK_HOME points outside
    ``~/.rok``, profiles live under ``ROK_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_rok_home() / "profiles"


def _get_default_rok_home() -> Path:
    """Return the default (pre-profile) ROK_HOME path.

    In standard deployments this is ``~/.rok``.
    In Docker/custom deployments where ROK_HOME is outside ``~/.rok``
    (e.g. ``/opt/data``), returns ROK_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_rok_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Rok installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ROK_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_rok_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, rok subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _ROK_SUBCOMMANDS:
        return f"'{canon}' conflicts with a rok subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``rok profile list`` for an unrelated profile.
    """
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_rok_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``rok update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.rok)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.rok/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `rok update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`rok profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `rok update`.\n",
                encoding="utf-8",
            )
        except OSError:
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ROK_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``rok profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ROK_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.rok).\n"
            "To remove everything, use: rok uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ROK_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ROK_HOME")
    try:
        os.environ["ROK_HOME"] = str(profile_dir)
        from rok_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ROK_HOME"] = old_home
        elif "ROK_HOME" in os.environ:
            del os.environ["ROK_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.rok/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: rok profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ROK_HOME.

    Returns ``"default"`` if ROK_HOME is not set or points to ``~/.rok``.
    Returns the profile name if ROK_HOME points into ``~/.rok/profiles/<name>``.
    Returns ``"custom"`` if ROK_HOME is set to an unrecognized path.
    """
    from rok_constants import get_rok_home
    rok_home = get_rok_home()
    resolved = rok_home.resolve()

    default_resolved = _get_default_rok_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.rok itself — its parent is ~/ and its
        # directory name is ".rok", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: rok profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.rok itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.rok). "
            "Specify a different name: rok profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="rok_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"rok.{old_name}"
    new_host = f"rok.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_rok_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ROK_HOME path string.

    Called early in the CLI entry point, before any rok modules
    are imported, to set the ROK_HOME environment variable.
    """
