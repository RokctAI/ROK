...
Usage::
    ...
    rok profile create coder --clone  # also copy config, .env, SOUL.md
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if name in _ROK_SUBCOMMANDS:
        return f"'{name}' conflicts with a rok subcommand"
...
def set_active_profile(name: str) -> None:
    ...
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            ...
            f"Create it with: rok profile create {name}"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for rok profile names."""
    return '''# Rok Agent profile completion
# Add to ~/.bashrc: eval "$(rok completion bash)"

_rok_profiles() {
    local profiles_dir="$HOME/.rok/profiles"
...
_rok_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_rok_profiles)" -- "$cur"))
        return
    fi
...
complete -F _rok_completion rok
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for rok profile names."""
    return '''#compdef rok
# Rok Agent profile completion
# Add to ~/.zshrc: eval "$(rok completion zsh)"

_rok() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.rok/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.rok/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: rok profile create {profile_name}"
        )
