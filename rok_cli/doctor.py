"""
Doctor command for rok CLI.

Diagnoses issues with Rok Agent setup.
"""
...
from rok_cli.config import get_project_root, get_rok_home, get_env_path
from rok_cli.env_loader import load_rok_dotenv
from rok_constants import display_rok_home

PROJECT_ROOT = get_project_root()
ROK_HOME = get_rok_home()
_DHH = display_rok_home()  # user-facing display path (e.g. ~/.rok or ~/.rok/profiles/coder)

# Load environment variables from ~/.rok/.env so API key checks work
_env_path = get_env_path()
load_rok_dotenv(rok_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from rok_cli.colors import Colors, color
from rok_cli.models import _ROK_USER_AGENT
from rok_cli.vercel_auth import describe_vercel_auth
from rok_constants import OPENROUTER_MODELS_URL
...
from rok_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.rok/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("ROK_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ROK_KANBAN_TASK"):
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from rok_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `rok`.
    os.environ.setdefault("ROK_INTERACTIVE", "1")

    # Handle `rok doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.rok/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Rok Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`rok doctor --ack {hit.advisory.id}`."
                )
    ...
    # Check ~/.rok/.env (primary location for user config)
    env_path = ROK_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'rok setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'rok setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'rok setup' to create one")
                issues.append("Run 'rok setup' to create .env")
    
    # Check ~/.rok/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = ROK_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from rok_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from rok_cli.providers import (
            ...
            if provider and provider != "auto":
                if catalog_provider is None or (
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'rok config set model.provider <valid_provider>'"
                    )
            ...
            if runtime_provider and runtime_provider not in {"auto", "custom", "openrouter"}:
                try:
                    from rok_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.rok/.env or run 'rok setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'rok setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'rok config set model.provider <name>'"
                            )
                except Exception:
                    pass
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from rok_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from rok_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'rok setup' to migrate config")
                else:
                    issues.append("Run 'rok doctor --fix' or 'rok setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'rok doctor --fix'")
        except Exception:
            pass
    ...
    if _safe_which("codex"):
        ...
        # Native OAuth uses Rok' own device-code flow — the Codex CLI is
        # only needed if you want to import existing tokens from
        # ~/.codex/auth.json.  Downgrade to info so users running
        # `rok auth openai-codex` aren't told they're missing something.
        check_info(
    ...
    for subdir_name in expected_subdirs:
        subdir_path = rok_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Rok a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Rok Agent Persona\n\n"
                "<!-- Edit this file to customize how Rok communicates. -->\n\n"
                "You are Rok, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = rok_home / "memories"
    ...
    state_db_path = rok_home / "state.db"
    ...
    wal_path = rok_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'rok doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (rok bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "rok"
        ...
        _cmd_link = _cmd_link_dir / "rok"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(rok not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/rok → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/rok points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/rok → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/rok — run 'rok doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/rok exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/rok not found",
                    "(rok command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/rok → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/rok symlink — run 'rok doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'rok-agent[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'rok-agent[vercel]'")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `rok doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'rok config set model.provider <provider>' "
    ...
    # never the legitimate source for `rok doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'rok setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: rok skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = ROK_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: rok memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: rok memory setup")
                issues.append("No Honcho API key — run 'rok memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run rok memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: rok memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: rok memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from rok_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "rok -p" in content:
                            _m = _re.search(r"rok -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'rok doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
