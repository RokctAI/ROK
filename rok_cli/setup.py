...
_DOCS_BASE = "https://rok.rokctai.com/docs"
...
def _setup_copilot_reasoning_selection(
    ...
    *,
    ...
    from rok_cli.models import github_model_reasoning_efforts, normalize_copilot_model_id
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Rok via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("ROK_MAX_ITERATIONS", "90")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("ROK_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("ROK_MAX_ITERATIONS", str(max_iter))
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://rok.rokctai.com/docs/user-guide/messaging/slack/")
    print()
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'rok whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'rok whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from rok_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from rok_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from rok_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from rok_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from rok_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from rok_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from rok_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from rok_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://rok.rokctai.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://rok.rokctai.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        if choice == 0:
            ...
            print_info("Exiting. Run 'rok setup' again when ready.")
            return
        elif 2 <= choice <= 6:
...
def _resolve_rok_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``rok chat`` in a fresh process."""
    ...
    try:
        if importlib.util.find_spec("rok_cli") is not None:
            return [sys.executable, "-m", "rok_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch rok chat now?", True):
        return

    chat_argv = _resolve_rok_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch Rok automatically. Run 'rok chat' manually.")
        return
