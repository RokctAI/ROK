"""
Backup and import commands for rok CLI.

`rok backup` creates a zip archive of the entire ~/.rok/ directory
(excluding the rok-agent repo and transient files).

`rok import` restores from a backup zip, overlaying onto the current
ROK_HOME root.
"""
...
from rok_constants import get_default_rok_root, get_rok_home, display_rok_home
...
_EXCLUDED_DIRS = {
    "rok-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to rok root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Rok home directory."""
    rok_root = get_default_rok_root()

    if not rok_root.is_dir():
        print(f"Error: Rok home directory not found at {rok_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"rok-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"rok-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_rok_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(rok_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(rok_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(rok_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside rok root
            try:
    ...
    print(f"\nRestore with: rok import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Rok backup.
    ...
    # Look for telltale files that a rok home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Rok backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.rok/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a rok dir name
        if prefix in {".rok", "rok"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_rok_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (rok_root / "config.yaml").exists()
        has_env = (rok_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Rok configuration.")
        ...
        for member in members:
            ...
            try:
                target.resolve().relative_to(rok_root.resolve())
            except ValueError:
        ...
        profiles_dir = rok_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # rok_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: rok profile list  (after installing rok)")

        # Guidance
        print()
        if not (rok_root / "rok-agent").is_dir():
            print("Note: The rok-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: rok update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  rok -p {pname} gateway install")

        print("Done. Your Rok configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and rok backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ROK_HOME).
...
# — `rok update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(rok_home: Optional[Path] = None) -> Path:
    home = rok_home or get_rok_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    rok_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(rok_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(rok_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for rok backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_rok_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, rok_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``rok_root`` to ``out_path``.
...
def _pre_update_backup_dir(rok_home: Optional[Path] = None) -> Path:
    home = rok_home or get_rok_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ROK_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ROK_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``rok update``) should continue even if the backup fails.
    """
    rok_root = rok_home or get_default_rok_root()
    if not rok_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(rok_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, rok_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `rok claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ROK_HOME under ``backups/`` before a
    ``rok claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``rok import <archive>``.  Writes to
    ``<ROK_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `rok import` and the
