...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ROK_HOME", os.path.expanduser("~/.rok")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ROK_HOME", Path.home() / ".rok")) / "skills" / "red-teaming" / "godmode"
...
ROK_HOME = Path(os.getenv("ROK_HOME", Path.home() / ".rok"))
CONFIG_PATH = ROK_HOME / "config.yaml"
PREFILL_PATH = ROK_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Rok models — already uncensored, just needs clean prompt
    "rok": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "rok" in model_lower or "nous" in model_lower:
        return "rok"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Rok config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.rok/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Rok for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Rok for changes to take effect.")
