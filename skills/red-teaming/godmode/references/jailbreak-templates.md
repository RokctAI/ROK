...
## 5. GODMODE FAST — Rok 4 405B (Zero Refusal)

**Model:** `rokctai/rok-4-405b`
**Strategy:** Rok models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Rok Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.rok/config.yaml`:
