#!/usr/bin/env python3
"""Google Workspace API CLI for Rok Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Rok-facing JSON contract and falls back to the Python client
...
from _rok_home import get_rok_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ROK_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Rok Agent")
