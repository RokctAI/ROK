...
Maintains the same CLI interface for backward compatibility with Rok skills.
...
def gws(*args: str) -> None:
    ...
    result = subprocess.run(
        ...
        env={**os.environ, "ROK_HOME": os.environ.get("ROK_HOME", str(Path.home() / ".rok"))},
    )
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Rok Agent (gws backend)")
