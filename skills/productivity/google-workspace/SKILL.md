...
description: Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via Python. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Python client libraries in the Rok venv.
version: 1.0.0
...
```bash
GSETUP="python ~/.rok/skills/productivity/google-workspace/scripts/setup.py"
```
...
```bash
GAPI="python ~/.rok/skills/productivity/google-workspace/scripts/google_api.py"
```
