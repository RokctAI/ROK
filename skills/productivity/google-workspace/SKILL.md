...
- `gws_bridge.py` refreshes the Rok token and injects it into `gws` via `GOOGLE_WORKSPACE_CLI_TOKEN`
...
GWORKSPACE_SKILL_DIR="$ROK_HOME/skills/productivity/google-workspace"
PYTHON_BIN="${ROK_PYTHON:-python3}"
if [ -x "$ROK_HOME/rok/venv/bin/python" ]; then
  PYTHON_BIN="$ROK_HOME/rok/venv/bin/python"
fi
