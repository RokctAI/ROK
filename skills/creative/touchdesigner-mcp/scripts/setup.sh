...
ROK_HOME_DIR="${ROK_HOME:-$HOME/.rok}"
ROK_CFG="${ROK_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Rok config has twozero_td MCP entry ──
if [[ ! -f "$ROK_CFG" ]]; then
    echo -e " ${FAIL} Rok config not found at ${ROK_CFG}"
    manual_steps+=("Create ${ROK_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ROK_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Rok config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Rok config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ROK_CFG} manually"); }
    manual_steps+=("Restart Rok session to pick up config change")
fi
