...
One ``CDPSupervisor`` runs per Rok ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "rok-dialog-bridge.invalid"
...
(() => {
  if (window.__rokDialogBridgeInstalled) return;
  window.__rokDialogBridgeInstalled = true;
  const ENDPOINT = "http://rok-dialog-bridge.invalid/";
