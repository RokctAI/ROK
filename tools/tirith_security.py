...
it is automatically downloaded from GitHub releases to $ROK_HOME/bin/tirith.
...
def _get_rok_home() -> str:
    """Return the Rok home directory, respecting ROK_HOME env var."""
    return str(get_rok_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_rok_home(), ".tirith-install-failed")
...
def _rok_bin_dir() -> str:
    """Return $ROK_HOME/bin, creating it if needed."""
    d = os.path.join(_get_rok_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ROK_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_rok_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ROK_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ROK_HOME/bin/tirith
    ...
    rok_bin = os.path.join(_rok_bin_dir(), "tirith")
    if os.path.isfile(rok_bin) and os.access(rok_bin, os.X_OK):
        _resolved_path = rok_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return rok_bin
