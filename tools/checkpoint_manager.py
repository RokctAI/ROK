...
Architecture:
    ~/.rok/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        ROK_WORKDIR                               — original dir path
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "rok@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "Rok Checkpoint"], shadow_repo, working_dir)
