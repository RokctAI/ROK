...
`mcp_tool.py`, and `rok_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# RokMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_rok_provider_class() -> Optional[type]:
    ...
    class RokMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._rok_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`RokTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, RokTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import RokTokenStorage
                        if isinstance(storage, RokTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._rok_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import RokTokenStorage
            if not isinstance(storage, RokTokenStorage):
                return
    ...
    return RokMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ROK_PROVIDER_CLS: Optional[type] = _make_rok_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`RokMCPOAuthProvider` directly using the helpers
        ...
        storage = RokTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``rok mcp remove <name>`` and (indirectly) by
        ``rok mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
