...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["ROK_RPC_SOCKET"])
...
def execute_code(
    ...
    try:
        ...
        with open(os.path.join(tmpdir, "rok_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["ROK_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Ensure the rok root is importable in the sandbox so
        ...
        child_env["PYTHONPATH"] = _rok_root + (os.pathsep + _existing_pp if _existing_pp else "")
