...
_OUTPUT_FENCE = "__ROK_FENCE_a9f7b3__"

# Rok-internal env vars that should NOT leak into terminal subprocesses.
# These are loaded from ~/.rok/.env for Rok' own LLM/provider calls
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    optional config registry, so new Rok-managed secrets are blocked
    ...
    # Vars not covered above but still Rok-internal / conflict-prone.
    blocked.update({
        "OPENAI_BASE_URL",
        "OPENAI_API_KEY",
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    `_ROK_FORCE_<VAR>` entries in ``extra_env`` opt a blocked variable back in
...
class LocalEnvironment(PersistentShellMixin, BaseEnvironment):
    ...
    def _temp_prefix(self) -> str:
        return f"/tmp/rok-local-{self._session_id}"
    ...
    def _execute_oneshot(self, command: str, cwd: str = "", *,
        ...
        user_shell = _find_bash()
        # Newline-separated wrapper (not `cmd; __rok_rc=...` on one line).
        # A trailing `; __rok_rc` glued to `<<EOF` / a closing `EOF` line breaks
