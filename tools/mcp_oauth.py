...
``httpx.Auth``) with Rok-specific token storage and browser-based
...
# ---------------------------------------------------------------------------
# Token storage — persists tokens + client info to ~/.rok/mcp-tokens/
# ---------------------------------------------------------------------------
...
class HermesTokenStorage:
    ...
    def _base_dir(self) -> Path:
        home = Path(os.environ.get("ROK_HOME", Path.home() / ".rok"))
...
async def _wait_for_callback() -> tuple[str, str | None]:
    ...
    connecting and potentially hanging Rok startup indefinitely.
    """
    global _oauth_port
    ...
    if not code:
        raise OAuthNonInteractiveError(
            ...
            "Run 'rok mcp auth <server-name>' to authorize interactively."
        )
    return code, state
    ...
    if not interactive:
        ...
        if not has_cached:
            logger.warning(
                ...
                "connect. Run 'rok mcp auth %s' to authorize interactively.",
    ...
    if not interactive:
        ...
        async def _noninteractive_callback() -> tuple[str, str | None]:
            raise OAuthNonInteractiveError(
                ...
                f"(no TTY). Run 'rok mcp auth {server_name}' to "
