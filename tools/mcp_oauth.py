...
This module provides the glue:
    - ``RokTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Rok Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses ROK_HOME so each profile gets its own OAuth tokens.
    Layout: ``ROK_HOME/mcp-tokens/``
    """
    try:
        from rok_constants import get_rok_home
        base = Path(get_rok_home())
    except ImportError:
        base = Path(os.environ.get("ROK_HOME", str(Path.home() / ".rok")))
...
# ---------------------------------------------------------------------------
# RokTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class RokTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ROK_HOME/mcp-tokens/<server_name>.json         -- tokens
        ROK_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ROK_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Rok records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Rok.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Rok Agent")
    scope = cfg.get("scope")
