...
discovers their tools, and registers them into the rok-agent tool registry
...
class MCPServerTask:
    ...
    async def _refresh_tools(self):
        ...
        async with self._refresh_lock:
            ...
            # 2. Remove old tools from rok-* umbrella toolsets
            for ts_name, ts in TOOLSETS.items():
                if ts_name.startswith("rok-"):
...
def _sync_mcp_toolsets(server_names: Optional[List[str]] = None) -> None:
    """Expose each MCP server as a standalone toolset and inject into rok-* sets.
    ...
    Also injects all MCP tools into rok-* umbrella toolsets for the
    default behavior.
    ...
    # Also inject into rok-* umbrella toolsets for default behavior.
    for ts_name, ts in TOOLSETS.items():
        if not ts_name.startswith("rok-"):
            continue
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> List[str]:
    ...
    and rok-* umbrella toolset injection.
    ...
    if registered_names:
        ...
        # Inject into rok-* umbrella toolsets for default behavior
        for ts_name, ts in TOOLSETS.items():
            if ts_name.startswith("rok-"):
