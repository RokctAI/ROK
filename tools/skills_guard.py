...
Trust levels:
  - builtin:   Ships with Rok. Never scanned, always trusted.
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.rok/\.env|\~/\.rok/\.env',
     "rok_env_access", "critical", "exfiltration",
     "directly references Rok secrets file"),
    ...
    (r'\.rok/config\.yaml|\.rok/SOUL\.md',
     "rok_config_mod", "critical", "persistence",
     "references Rok configuration files directly"),
...
def _get_configured_model() -> str:
    """Load the user's configured model from ~/.rok/config.yaml."""
    try:
