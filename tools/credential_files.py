...
Each registered entry is a dict::
    ...
    {
        "host_path": "/home/user/.rok/google_token.json",
        "container_path": "/root/.rok/google_token.json",
    }
"""
...
def _resolve_rok_home() -> Path:
    return Path(os.environ.get("ROK_HOME", Path.home() / ".rok"))


def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.rok",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``ROK_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside ROK_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    rok_home = _resolve_rok_home()

    # Reject absolute paths — they bypass the ROK_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to ROK_HOME)",
            relative_path,
        )
        return False

    host_path = rok_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape ROK_HOME.
    try:
        resolved = host_path.resolve()
        rok_home_resolved = rok_home.resolve()
        resolved.relative_to(rok_home_resolved)  # raises ValueError if outside
    except ValueError:
        logger.warning(
            "credential_files: rejected path traversal %r "
            "(resolves to %s, outside ROK_HOME %s)",
            relative_path,
            resolved,
            rok_home_resolved,
        )
                ...
                for item in cred_files:
                    if isinstance(item, str) and item.strip():
                        ...
                        if os.path.isabs(rel):
                            ...
                            continue
                        host_path = (rok_home / rel).resolve()
                        try:
                            host_path.relative_to(rok_home_resolved)
                        except ValueError:
                            logger.warning(
                                "credential_files: rejected config path traversal %r "
                                "(resolves to %s, outside ROK_HOME %s)",
                                rel, host_path, rok_home_resolved,
                            )
                            continue
                        if host_path.is_file():
                            container_path = f"/root/.rok/{rel}"
                            result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="rok-skills-safe-"))
