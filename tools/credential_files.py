...
def register_credential_file(
    ...
    *relative_path* is relative to ``ROK_HOME`` (e.g. ``google_token.json``).
    ...
    containment_error = validate_within_dir(host_path, rok_home)
...
# Each tuple is (new_subpath, old_name) matching rok_constants.get_rok_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_rok_dir()`` for backward compatibility with old directory layouts.
    """
    from rok_constants import get_rok_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_rok_dir(new_subpath, old_name)
        if host_dir.is_dir():
