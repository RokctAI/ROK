...
Backend is selected during ``rok tools`` setup (web.backend in config.yaml).
When available, Rok can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.rok/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``rok tools``).
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if managed_nous_tools_enabled():
        message += (
            ...
            "(`rok model`) and provide FIRECRAWL_GATEWAY_URL or TOOL_GATEWAY_DOMAIN."
        )
...
def _get_firecrawl_client():
    ...
    Rok falls back to the Firecrawl tool-gateway for logged-in Nous Subscribers.
    """
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    ...
    return host == "rokctai.com" or host.endswith(".rokctai.com")
