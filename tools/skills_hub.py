#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Rok Skills Hub.
...
Used by rok_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            rok_meta = metadata.get("rok", {})
            if isinstance(rok_meta, dict):
                tags = rok_meta.get("tags", [])
        if not tags:
...
# ---------------------------------------------------------------------------
# Rok centralized index source
# ---------------------------------------------------------------------------

ROK_INDEX_URL = "https://rok.rokct.ai/docs/api/skills-index.json"
ROK_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "rok-index.json"
ROK_INDEX_TTL = 6 * 3600  # 6 hours


def _load_rok_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ROK_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ROK_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ROK_INDEX_CACHE_FILE.stat().st_mtime
            if age < ROK_INDEX_TTL:
                return json.loads(ROK_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ROK_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Rok index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Rok index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ROK_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ROK_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class RokIndexSource(SkillSource):
    """Skill source backed by the centralized Rok Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "rok-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "rok-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        RokIndexSource(auth=auth), # Centralized index (search + resolved install paths)
