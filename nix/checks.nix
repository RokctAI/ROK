...
' > $out
    ...
    in {
      ...
      checks = lib.optionalAttrs pkgs.stdenv.hostPlatform.isLinux {
        ...
        cli-commands = pkgs.runCommand "rok-cli-commands" { } ''
          set -e
        ...
        # Verify ROK_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "rok-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ROK_MANAGED=true "$@" 2>&1 || true)
        ...
        in pkgs.runCommand "rok-config-roundtrip" {
