...
#
# Container mode: rok runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ROK_HOME/.env
# and read by rok at startup — no container recreation needed for env changes.
#
# Tool resolution: the rok wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    generatedConfigFile = pkgs.writeText "rok-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "rok-documents" { } (
      ''
        mkdir -p $out
    ...
    # rok user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "rok-container-entrypoint" ''
      set -eu

      ROK_UID="''${ROK_UID:?ROK_UID must be set}"
      ROK_GID="''${ROK_GID:?ROK_GID must be set}"

      # ── Group: ensure a group with GID=$ROK_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ROK_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="rok"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ROK_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ROK_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ROK_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ROK_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="rok"
        TARGET_HOME="/home/rok"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ROK_UID" -g "$ROK_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ROK_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      ...
      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/rok ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/rok
        chmod 0440 /etc/sudoers.d/rok
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ROK_UID" --regid="$ROK_GID" --init-groups "$@"
    ...
    # Default: /var/lib/rok/workspace → /data/workspace.
  ...
  in {
    options.services.rok-agent = with lib; {
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/rok";
        description = "State directory. Contains .rok/ subdir (ROK_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Rok config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ROK_HOME/.env at activation time.
          Rok reads this file on every startup via load_rok_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ROK_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ROK_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `rok gateway`.";
      };
      ...
      # ── Activation: link config + auth + documents ────────────────────
      {
        system.activationScripts."rok-agent-setup" = lib.stringAfter [ "users" ] ''
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.rok/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.rok/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.rok/config.yaml
            chmod 0640 ${cfg.stateDir}/.rok/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.rok/.managed
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.rok/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.rok/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.rok/auth.json
              fi
            ''}
          ''}
          ...
          # Rok reads $ROK_HOME/.env at startup via load_rok_dotenv(),
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.rok-agent = {
          ...
          serviceConfig = {
            ...
            # $ROK_HOME/.env by the activation script. load_rok_dotenv()
        ...
        systemd.services.rok-agent = {
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ROK_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ROK_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ROK_UID="$ROK_UID" \
                --env ROK_GID="$ROK_GID" \
                --env ROK_HOME=${containerDataDir}/.rok \
                --env ROK_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/rok gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
