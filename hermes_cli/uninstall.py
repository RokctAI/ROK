"""
Rok Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.rok/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Rok PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                ...
                # Remove any PATH line containing rok
                if 'rok' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the rok wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "rok",
        Path("/usr/local/bin/rok"),
    ]
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.rok/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.rok/ for future reinstall
    """
    ...
    print(f"  Config:  {rok_home / 'config.yaml'}")
    ...
    if full_uninstall:
        ...
        try:
            if rok_home.exists():
                shutil.rmtree(rok_home)
                log_success(f"Removed {rok_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {rok_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {rok_home}")
