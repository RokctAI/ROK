...
from rok_cli.config import get_rok_home, get_config_path
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `rok login` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `rok login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `rok login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `rok login` to re-authenticate.",
            provider="openai-codex",
    ...
    *,
    ...
    try:
        ...
        if cli_tokens:
            ...
            print("   Run `rok login` to create a fully independent session.\n")
...
def detect_external_credentials() -> List[Dict[str, Any]]:
    """Scan for credentials from other CLI tools that Rok can reuse.

    Returns a list of dicts, each with:
      - provider: str   -- Rok provider id (e.g. "openai-codex")
    ...
    if cli_tokens:
        ...
        found.append({
            "provider": "openai-codex",
            "path": str(codex_path),
            "label": f"Codex CLI credentials found ({codex_path}) — run `rok login` to create a separate session",
        })
...
def login_command(args) -> None:
    ...
    print("Use 'rok model' to select a provider and model,")
    print("or 'rok setup' for full interactive setup.")
    raise SystemExit(0)
