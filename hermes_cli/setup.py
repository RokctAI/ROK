...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    """
    from rok_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
...
def setup_model_provider(config: dict):
    ...
    if selected_provider in _vision_backends:
        # If the user just selected a backend Rok can already use for
...
def setup_gateway(config: dict):
    ...
    if not existing_slack and prompt_yes_no("Set up Slack bot?", False):
        ...
        print()
        print_info(
            "   Full guide: https://rok-agent.rokctai.com/docs/user-guide/messaging/slack/"
        )
        print()
    ...
    if not existing_webhook and prompt_yes_no("Set up webhooks? (GitHub, GitLab, etc.)", False):
        ...
        print()
        print_info(
            "   Full guide: https://rok-agent.rokctai.com/docs/user-guide/messaging/webhooks/"
        )
        print()
        ...
        print_info(
            "   https://rok-agent.rokctai.com/docs/user-guide/messaging/webhooks/#configuring-routes"
        )
        print()
...
def _offer_openclaw_migration(rok_home: Path) -> bool:
    ...
    print_info("Rok can import your settings, memories, skills, and API keys.")
    print()
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Rok.")
    if skipped:
