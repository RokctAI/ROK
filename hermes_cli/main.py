...
def _apply_profile_override() -> None:
    ...
    # 2. If no flag, check ~/.rok/active_profile
    if profile_name is None:
        try:
            active_path = Path.home() / ".rok" / "active_profile"
...
def select_provider_and_model(args=None):
    ...
    from rok_cli.config import load_config, get_env_value
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from rok_cli.models import model_ids
...
def _update_via_zip(args):
    ...
    branch = "main"
    zip_url = f"https://github.com/RokctAI/rok-agent/archive/refs/heads/{branch}.zip"
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        do_reset = True
        if prompt_user:
            print("\nReset working tree to clean state so Rok can run?")
    ...
    if not has_upstream:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/RokctAI/rok-agent.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/RokctAI/rok-agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    default_home = Path.home() / ".rok"
    homes.append(default_home)
    # Named profiles under ~/.rok/profiles/
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "rok-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    ...
    try:
        ...
        # Uses the PID file (scoped to ROK_HOME) to find this
        ...
        try:
            ...
            # Also check for a system-level service (rok gateway install --system).
            ...
            if is_macos():
                try:
                    from rok_cli.gateway import get_launchd_label
            ...
            if existing_pid or has_systemd_service or has_system_service or has_launchd_service:
                ...
                if has_systemd_service:
                    ...
                    if restart.returncode == 0:
                        ...
                        if is_linux():
                            ...
                            if linger_ok is not True:
                                ...
                                print("    rok gateway restart")
                            else:
                                print("  Try manually: rok gateway restart")
                elif has_system_service:
                    # System-level service (rok gateway install --system).
                    ...
                    print("  Restart it with: rok gateway run")
        except Exception as e:
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if clone or clone_all:
                from rok_constants import get_rok_home
                profile_dir_display = f"~/.rok/profiles/{name}"
...
Examples:
    ...
    rok auth remove <p> <n>    Remove pooled credential by index
...
"""
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Rok Agent with an interactive wizard. "
                    "Run a specific section: rok setup model|terminal|gateway|tools|agent"
    )
    ...
    # Provider selection happens via 'rok memory setup'.
    # =========================================================================
    honcho_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "plugin — initial setup is via 'rok memory setup'. These commands\n"
    ...
    honcho_subparsers.add_parser("setup", help="Initial Honcho setup (redirects to rok memory setup)")
    ...
    honcho_subparsers.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Rok Honcho",
    )
    ...
    def cmd_honcho(args):
        ...
        if sub == "setup":
            ...
            print("  Running 'rok memory setup'...\n")
