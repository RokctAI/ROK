...
def _get_profiles_root() -> Path:
    ...
    Always ``~/.rok/profiles/`` — anchored to the user's home,
    NOT to the current ROK_HOME (which may itself be a profile).
    This ensures ``coder profile list`` can see all profiles.
    """
    return Path.home() / ".rok" / "profiles"


def _get_default_rok_home() -> Path:
    """Return the default (pre-profile) ROK_HOME path."""
    return Path.home() / ".rok"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    # Check for service
    from rok_cli.gateway import _profile_suffix, get_service_name
    wrapper_path = _get_wrapper_dir() / name
