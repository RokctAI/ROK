"""rok webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.rok/webhook_subscriptions.json and are
...
def _rok_home() -> Path:
    return Path(
        os.getenv("ROK_HOME", str(Path.home() / ".rok"))
    ).expanduser()


def _subscriptions_path() -> Path:
    return _rok_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: rok gateway run
"""
...
def webhook_command(args):
    """Entry point for 'rok webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: rok webhook {subscribe|list|remove|test}")
        print("Run 'rok webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (rok gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: rok webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from rok webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (rok gateway run)")
