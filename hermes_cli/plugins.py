...
1. **User plugins**   – ``~/.rok/plugins/<name>/``
2. **Project plugins** – ``./.rok/plugins/<name>/`` (opt-in via
   ``ROK_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``rok_agent.plugins``
   entry-point group.
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.rok/plugins/)
        rok_home = os.environ.get("ROK_HOME", os.path.expanduser("~/.rok"))
        user_dir = Path(rok_home) / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.rok/plugins/)
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``rok_plugins.<name>``."""
