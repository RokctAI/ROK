...
from rok_cli.config import get_env_value, get_rok_home, save_env_value, is_managed, managed_error
...
def _profile_suffix() -> str:
    ...
    Returns ``""`` for the default ``~/.rok``, the profile name for
    ``~/.rok/profiles/<name>``, or a short hash for any other custom
    ROK_HOME path.
    """
    import hashlib
    import re
    ...
    default = (_Path.home() / ".rok").resolve()
    if home == default:
        return ""
    # Detect ~/.rok/profiles/<name> pattern → use the profile name
...
def get_rok_cli_path() -> str:
    """Get the path to the rok CLI."""
    ...
    return f"{get_python_path()} -m rok_cli.main"
...
User={username}
Group={group_name}
ExecStart={python_path} -m rok_cli.main gateway run --replace
    ...
    if any_configured:
        ...
        if service_running:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: rok gateway")
    else:
        print()
    ...
    if subcmd == "install":
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print()
                print("To start:")
                print("  rok gateway          # Run in foreground")
