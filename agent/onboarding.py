...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Rok starts and finds ``~/.openclaw/``.

    Points users at ``rok claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``rok claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Rok, run "
        "`rok claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `rok claw cleanup` (renames it to ~/.openclaw.pre-migration — "
