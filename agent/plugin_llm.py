...
Plugins built on Rok Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Rok
...
:class:`~rok_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Rok' main config
...
handles every provider, fallback chain, and per-task override Rok
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`rok_cli.plugins.PluginContext`
