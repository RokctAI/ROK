...
Data is cached in memory (1hr TTL) and on disk (~/.rok/models_dev_cache.json)
...
# Provider ID mapping: Rok provider names → models.dev provider IDs
...
def _get_cache_path() -> Path:
    ...
    env_val = os.environ.get("ROK_HOME", "")
    rok_home = Path(env_val) if env_val else Path.home() / ".rok"
    return rok_home / "models_dev_cache.json"
