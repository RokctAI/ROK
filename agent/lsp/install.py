...
appropriate.  All installs go to a Rok-owned bin staging dir,
``<ROK_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``rok lsp
  status``.
...
# ``<ROK_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def rok_lsp_bin_dir() -> Path:
    """Return the Rok-owned bin staging dir for LSP servers."""
    home = os.environ.get("ROK_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".rok")
    p = Path(home) / "lsp" / "bin"
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = rok_lsp_bin_dir().parent  # <ROK_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a rok-owned target dir.
    ...
    console script.
    """
    pip_target = rok_lsp_bin_dir().parent / "python-packages"
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``rok lsp status`` CLI to give users a quick
