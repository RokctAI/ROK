...
Owns one Codex thread per Rok session. Drives `turn/start`, consumes
...
# Rok' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ROK_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Rok session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ROK_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ROK_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="rok",
            client_title="Rok Agent",
            client_version=_get_rok_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Rok' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Rok' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own rok-tools callback we
            # auto-accept — the user already approved Rok' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Rok approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Rok returns 'once', 'session', 'always', or 'deny'.
...
def _get_rok_version() -> str:
    """Best-effort Rok version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("rok-agent")
