...
from rok_constants import get_rok_home, get_skills_dir, is_wsl
...
_ROK_MD_NAMES = (".rok.md", "ROK.md")


def _find_rok_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.rok.md`` or ``ROK.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Rok Agent, an intelligent AI assistant created by Nous Research. "
...
ROK_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Rok Agent "
    "itself, load the `rok-agent` skill with skill_view(name='rok-agent') "
    "before answering. Docs: https://rok.rokct.ai/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.rok/kanban.db`. Your task id is in "
    "`$ROK_KANBAN_TASK`; your workspace is `$ROK_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ROK_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `rok kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Rok WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Rok itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Rok runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Rok itself is running. The host OS, home, and cwd "
                f"of the Rok process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Rok "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_rok_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.rok/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ROK_PLATFORM")
        or get_session_env("ROK_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Rok Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `rok-agent` skill "
            "first. It has the actual commands (e.g. `rok config set …`, `rok tools`, "
            "`rok setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: rok setup, rok setup tools, rok setup terminal, rok status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from ROK_HOME and return its content, or None.
    ...
    """
    try:
        from rok_cli.config import ensure_rok_home
        ensure_rok_home()
    except Exception as e:
        logger.debug("Could not ensure ROK_HOME before loading SOUL.md: %s", e)

    soul_path = get_rok_home() / "SOUL.md"
...
def _load_rok_md(cwd_path: Path) -> str:
    """.rok.md / ROK.md — walk to git root."""
    rok_md_path = _find_rok_md(cwd_path)
    if not rok_md_path:
        return ""
    try:
        content = rok_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = rok_md_path.name
        try:
            rel = str(rok_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".rok.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", rok_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .rok.md / ROK.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ROK_HOME is independent and always included when present.
    ...
    # SOUL.md from ROK_HOME only — skip when already loaded as identity
    if not skip_soul:
