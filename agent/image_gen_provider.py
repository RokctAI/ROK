...
as ``kind: backend``) or ``~/.rok/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$ROK_HOME/cache/images/``, creating parents as needed."""
    from rok_constants import get_rok_home

    path = get_rok_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$ROK_HOME/cache/images/``.
