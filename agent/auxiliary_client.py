...
Resolution order for text tasks (auto mode):
  ...
  2. Nous Portal (~/.rok/auth.json active provider)
...
NOUS_EXTRA_BODY = {"tags": ["product=rok"]}
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from rok_cli.models import check_nous_free_tier
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=rok"])
    if merged_extra:
