"""Windows UTF-8 bootstrap for Rok entry points.
...
should be imported at the very top of every Rok entry point
(``rok``, ``rok-agent``, ``rok-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import rok_bootstrap``
# (or ``from rok_bootstrap import apply_windows_utf8_bootstrap``) at
