...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs RokLoop with terminal tool only
...
class YCBenchEvalConfig(RokEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends RokEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(RokBaseEnv):
    """
    ...
    a competing built-in agent loop). The RokLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs RokLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our RokLoop.
