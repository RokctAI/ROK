...
## Token Lifecycle
- Store at `~/.rok/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `rok_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `rok_cli/models.py` — Add Gemini model catalog
4. `rok_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `rok_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `rok_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
- OR: accept user-provided client_id via ROK_GEMINI_CLIENT_ID env var
