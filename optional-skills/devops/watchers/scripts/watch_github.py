...
Usage (via cron with --no-agent):
    ...
    rok cron create rok-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ROK_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name rok-issues --repo Rokctai/rok-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.rok/.env to avoid the 60 req/hr
