...
Usage (via cron with --no-agent):
    ...
    rok cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ROK_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Rok-Watcher/1.0"})
