...
Maps each role archetype to the Rok skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public rok-agent
...
## Rok skills relevant to video production

### Visual / rendering skills (`rok-agent/skills/creative/`)
...
### Design / pre-production skills (`rok-agent/skills/creative/`)
...
### Audio / media skills (`rok-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`rok-agent/skills/devops/`)
...
These are **not** Rok skills but external CLIs / APIs that profiles invoke.
...
## Built-in Rok tools for media review

These are native Rok tools — not invoked via terminal but through their own
...
key is present in `~/.rok/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Rok profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.rok/.env` → environment variable.
...
When a new Rok-public video skill ships:
