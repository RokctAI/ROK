#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `rok kanban list` and
...
def rok_available() -> bool:
    return shutil.which("rok") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed rok CLI."""
    try:
        out = subprocess.run(
            ["rok", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `rok kanban list`
    out = subprocess.run(
        ["rok", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not rok_available():
        print("ERROR: 'rok' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
