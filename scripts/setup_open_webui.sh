...
# Bootstrap Open WebUI against Rok Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.rok/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-rok.sh
...
#   ROK_API_PORT=8642
#   ROK_API_HOST=127.0.0.1
#   ROK_API_MODEL_NAME='Rok Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Rok Agent WebUI}"
...
ROK_ENV_FILE="${ROK_ENV_FILE:-$HOME/.rok/.env}"
ROK_API_PORT="${ROK_API_PORT:-8642}"
ROK_API_HOST="${ROK_API_HOST:-127.0.0.1}"
ROK_API_CONNECT_HOST="${ROK_API_CONNECT_HOST:-127.0.0.1}"
ROK_API_MODEL_NAME="${ROK_API_MODEL_NAME:-Rok Agent}"
ROK_API_BASE_URL="http://${ROK_API_CONNECT_HOST}:${ROK_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-rok.sh"
LOG_DIR="$HOME/.rok/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$ROK_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$ROK_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.rok.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.rok</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.rok"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.rok"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-rok.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Rok Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-rok.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.rok/logs/openwebui.log
StandardError=append:%h/.rok/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-rok.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$ROK_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Rok API server is configured...'
  upsert_env API_SERVER_ENABLED true "$ROK_ENV_FILE"
  upsert_env API_SERVER_HOST "$ROK_API_HOST" "$ROK_ENV_FILE"
  upsert_env API_SERVER_PORT "$ROK_API_PORT" "$ROK_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$ROK_API_MODEL_NAME" "$ROK_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$ROK_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Rok gateway so API server settings take effect...'
  rok gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${ROK_API_CONNECT_HOST}:${ROK_API_PORT}/health" >/dev/null; then
    log 'Rok API server did not answer on the first check. Trying to start gateway in the background...'
    nohup rok gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${ROK_API_CONNECT_HOST}:${ROK_API_PORT}/health" >/dev/null
  ...
  log "Rok API endpoint: ${ROK_API_BASE_URL}"
