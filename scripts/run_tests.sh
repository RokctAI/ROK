#!/usr/bin/env bash
# Canonical test runner for rok-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.rok/rok-agent/venv"; do
...
# Unset ROK_* behavioral vars too.
unset ROK_YOLO_MODE ROK_INTERACTIVE ROK_QUIET ROK_TOOL_PROGRESS \
      ROK_TOOL_PROGRESS_MODE ROK_MAX_ITERATIONS ROK_SESSION_PLATFORM \
      ROK_SESSION_CHAT_ID ROK_SESSION_CHAT_NAME ROK_SESSION_THREAD_ID \
      ROK_SESSION_SOURCE ROK_SESSION_KEY ROK_GATEWAY_SESSION \
      ROK_CRON_SESSION \
      ROK_PLATFORM ROK_INFERENCE_PROVIDER ROK_MANAGED ROK_DEV \
      ROK_CONTAINER ROK_EPHEMERAL_SYSTEM_PROMPT ROK_TIMEZONE \
      ROK_REDACT_SECRETS ROK_BACKGROUND_NOTIFICATIONS ROK_EXEC_ASK \
      ROK_HOME_MODE 2>/dev/null || true
...
# If a system-wide rok pytest_live_guard plugin is installed at
# $HOME/.rok/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.rok/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.rok:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.rok" ;;
  esac
