...
VERSION_FILE = REPO_ROOT / "rok_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+rok-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@rokctai.com": "shannonsands",
    ...
    "rok@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (ROK_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (ROK_HOME inject)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"rok-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "rok@rokct.ai"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/RokctAI/rok-agent",
    ...
    lines.append(f"# Rok Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Rok Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Rok Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Rok Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Rok Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Rok Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Rok Agent v{new_version} ({calver_date})' "
