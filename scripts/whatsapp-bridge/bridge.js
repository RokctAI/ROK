...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.rok', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.rok', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.rok', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.rok', 'audio_cache');
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `rok_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
