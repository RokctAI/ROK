 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.rok/whatsapp/session
 */
...
const SESSION_DIR = getArg(
  "session",
  path.join(process.env.HOME || "~", ".rok", "whatsapp", "session"),
);
...
const DOCUMENT_CACHE_DIR = path.join(
  process.env.HOME || "~",
  ".rok",
  "document_cache",
);
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ["Rok Agent", "Chrome", "120.0"],
    ...
    if (connection === "close") {
      ...
      if (reason === DisconnectReason.loggedOut) {
        ...
        if (reason === 515) {
          ...
          console.log(
            `⚠️  Connection closed (reason: ${reason}). Reconnecting in 3s...`,
          );
        }
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      const quotedParticipant = normalizeWhatsAppId(
        contextInfo?.participant || contextInfo?.remoteJid || "",
      );
      ...
      if (messageContent.conversation) {
        ...
        try {
          const audioMsg =
            messageContent.pttMessage || messageContent.audioMessage;
          const buf = await downloadMediaMessage(
            msg,
            "buffer",
            {},
      ...
      // Ignore Rok' own reply messages in self-chat mode to avoid loops.
      if (
        msg.key.fromMe &&
...
app.post("/edit", async (req, res) => {
  ...
  if (!chatId || !messageId || !message) {
    return res
      .status(400)
      .json({ error: "chatId, messageId, and message are required" });
  }
...
if (PAIR_ONLY) {
  ...
  app.listen(PORT, "127.0.0.1", () => {
    console.log(
      `🌉 WhatsApp bridge listening on port ${PORT} (mode: ${WHATSAPP_MODE})`,
    );
    ...
    if (ALLOWED_USERS.size > 0) {
      ...
      console.log(
        `⚠️  No WHATSAPP_ALLOWED_USERS set — all messages will be processed`,
      );
    }
    console.log();
    startSocket();
