...
ROK_HOME="$HOME/.rok"
INSTALL_DIR="${ROK_INSTALL_DIR:-$ROK_HOME/rok-agent}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.rok/rok-agent)"
...
setup_path() {
    ...
    mkdir -p "$HOME/.local/bin"
    ln -sf "$ROK_BIN" "$HOME/.local/bin/rok"
    log_success "Symlinked rok → ~/.local/bin/rok"
...
maybe_start_gateway() {
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                ROK_CMD="$HOME/.local/bin/rok"
                [ ! -x "$ROK_CMD" ] && ROK_CMD="rok"
...
print_success() {
    ...
    echo -e "   ${YELLOW}Code:${NC}      ~/.rok/rok-agent/"
    echo ""
