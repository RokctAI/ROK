...
# Configuration
REPO_URL_SSH="git@github.com:Rokctai/rok.git"
REPO_URL_HTTPS="https://github.com/RokctAI/rok.git"
ROK_HOME="$HOME/.rok"
INSTALL_DIR="${ROK_INSTALL_DIR:-$ROK_HOME/rok}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.rok/rok)"
...
setup_path() {
    ...
    ln -sf "$ROK_BIN" "$command_link_dir/rok"
    log_success "Symlinked rok → $command_link_display_dir/rok"
...
copy_config_templates() {
    ...
    mkdir -p "$ROK_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Rok itself does not require root access."
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.rok/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.rok/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.rok/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.rok/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.rok/rok/"
    echo ""
