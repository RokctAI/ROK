...
Comparison of Rok Agent vs. openclaw-honcho — and a porting spec for bringing Rok patterns into other Honcho integrations.

---

## Overview

Two independent Honcho integrations have been built for two different agent runtimes: **Rok Agent** (Python, baked into the runner) and **openclaw-honcho** (TypeScript plugin via hook/tool API). Both use the same Honcho peer paradigm — dual peer model, `session.context()`, `peer.chat()` — but they made different tradeoffs at every layer.

This document maps those tradeoffs and defines a porting spec: a set of Rok-originated patterns, each stated as an integration-agnostic interface, that any Honcho integration can adopt regardless of runtime or language.

> **Scope** Both integrations work correctly today. This spec is about the delta — patterns in Rok that are worth propagating and patterns in openclaw-honcho that Rok should eventually adopt. The spec is additive, not prescriptive.
...
| Dimension | Rok Agent | openclaw-honcho |
|---|---|---|
...
| **AI peer name in identity** | Replaces "Rok Agent" in DEFAULT_AGENT_IDENTITY when configured. | Not implemented. |
...
Six patterns from Rok are worth adopting in any Honcho integration. Each is described as an integration-agnostic interface.
...
**openclaw-honcho contributes back (Rok should adopt):**
...
- [ ] **honcho mode / sessions / map** — CLI parity with Rok
...
Greenfield integration. Start from openclaw-honcho's architecture and apply all Rok patterns from day one.
