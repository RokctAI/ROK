# Migrating from OpenClaw to Rok Agent

This guide covers how to import your OpenClaw settings, memories, skills, and API keys into Rok Agent.

## Three Ways to Migrate

### 1. Automatic (during first-time setup)

When you run `rok setup` for the first time and Rok detects `~/.openclaw`, it automatically offers to import your OpenClaw data before configuration begins. Just accept the prompt and everything is handled for you.

### 2. CLI Command (quick, scriptable)

```bash
rok claw migrate                      # Preview then migrate (always shows preview first)
rok claw migrate --dry-run            # Preview only, no changes
rok claw migrate --preset user-data   # Migrate without API keys/secrets
rok claw migrate --yes                # Skip confirmation prompt
```
...
```
> Migrate my OpenClaw setup to Rok
```
...
|------|--------|-------------|
| SOUL.md | `~/.openclaw/workspace/SOUL.md` | `~/.rok/SOUL.md` |
| Memory entries | `~/.openclaw/workspace/MEMORY.md` | `~/.rok/memories/MEMORY.md` |
| User profile | `~/.openclaw/workspace/USER.md` | `~/.rok/memories/USER.md` |
| Skills | `~/.openclaw/workspace/skills/` | `~/.rok/skills/openclaw-imports/` |
| Command allowlist | `~/.openclaw/workspace/exec_approval_patterns.yaml` | Merged into `~/.rok/config.yaml` |
| Messaging settings | `~/.openclaw/config.yaml` (TELEGRAM_ALLOWED_USERS, MESSAGING_CWD) | `~/.rok/.env` |
| TTS assets | `~/.openclaw/workspace/tts/` | `~/.rok/tts/` |
...
| Item | Source | Destination |
|------|--------|-------------|
| Telegram bot token | `openclaw.json` channels config | `~/.rok/.env` |
| OpenRouter API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.rok/.env` |
| OpenAI API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.rok/.env` |
| Anthropic API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.rok/.env` |
| ElevenLabs API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.rok/.env` |
...
- **TTS provider**: OpenClaw renamed "edge" to "microsoft" — both are recognized and mapped to Rok' "edge"
...
By default, the migration **will not overwrite** existing Rok data:

- **SOUL.md** — skipped if one already exists in `~/.rok/`
...
- **API keys** — skipped if the key is already set in `~/.rok/.env`
...
For executed migrations, the full report is saved to `~/.rok/migration/openclaw/<timestamp>/`.

## Post-Migration Notes

- **Skills require a new session** — imported skills take effect after restarting your agent or starting a new chat.
- **WhatsApp requires re-pairing** — WhatsApp uses QR-code pairing, not token-based auth. Run `rok whatsapp` to pair.
- **Archive cleanup** — after migration, you'll be offered to rename `~/.openclaw/` to `.openclaw.pre-migration/` to prevent state confusion. You can also run `rok claw cleanup` later.
...
```bash
rok claw migrate --source /path/to/.openclaw
```

### "Migration script not found"
The migration script ships with Rok Agent. If you installed via pip (not git clone), the `optional-skills/` directory may not be present. Install the skill from the Skills Hub:
```bash
rok skills install openclaw-migration
```

### Memory overflow
If your OpenClaw MEMORY.md or USER.md exceeds Rok' character limits, excess entries are exported to an overflow file in the migration report directory. You can manually review and add the most important ones.
...
The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `rok config set`.
