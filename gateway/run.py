...
if _config_path.exists():
    try:
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            ...
            # Bridge agent.gateway_timeout → ROK_TIMEOUT env var.
            # Env var from .env takes precedence (already in os.environ).
            if "gateway_timeout" in _agent_cfg and "ROK_TIMEOUT" not in os.environ:
                os.environ["ROK_TIMEOUT"] = str(_agent_cfg["gateway_timeout"])
            if "gateway_timeout_warning" in _agent_cfg and "ROK_TIMEOUT_WARNING" not in os.environ:
                os.environ["ROK_TIMEOUT_WARNING"] = str(_agent_cfg["gateway_timeout_warning"])
            if "gateway_notify_interval" in _agent_cfg and "ROK_NOTIFY_INTERVAL" not in os.environ:
                os.environ["ROK_NOTIFY_INTERVAL"] = str(_agent_cfg["gateway_notify_interval"])
            if "restart_drain_timeout" in _agent_cfg and "ROK_RESTART_DRAIN_TIMEOUT" not in os.environ:
                os.environ["ROK_RESTART_DRAIN_TIMEOUT"] = str(_agent_cfg["restart_drain_timeout"])
        _display_cfg = _cfg.get("display", {})
        if _display_cfg and isinstance(_display_cfg, dict):
            if "busy_input_mode" in _display_cfg and "ROK_GATEWAY_BUSY_INPUT_MODE" not in os.environ:
                os.environ["ROK_GATEWAY_BUSY_INPUT_MODE"] = str(_display_cfg["busy_input_mode"])
        # Timezone: bridge config.yaml → ROK_TIMEZONE env var.
        # ROK_TIMEZONE from .env takes precedence (already in os.environ).
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str) and "ROK_TIMEZONE" not in os.environ:
...
def _expand_whatsapp_auth_aliases(identifier: str) -> set:
    ...
    session_dir = _rok_home / "whatsapp" / "session"
    resolved = set()
    queue = [normalized]
...
def _load_gateway_config() -> dict:
    ...
    try:
        ...
        logger.debug("Could not load gateway config from %s", _rok_home / 'config.yaml')
    return {}
...
class GatewayRunner:
    ...
    def _has_setup_skill(self) -> bool:
        """Check if the rok-setup skill is installed."""
        try:
            from tools.skill_manager_tool import _find_skill
            return _find_skill("rok-setup") is not None
        except Exception:
            return False
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            if not check_slack_requirements():
                logger.warning("Slack: slack-bolt not installed. Run: pip install 'rok[slack]'")
                return None
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        _raw_stale_timeout = float(os.getenv("ROK_TIMEOUT", 1800))
        ...
        if _quick_key in self._running_agents:
            ...
            from rok_cli.commands import resolve_command as _resolve_cmd_inner
        ...
        # in rok_cli/commands.py — no hardcoded set to maintain here.
        from rok_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command as _resolve_cmd
                    ...
                    if _stt_adapter:
                        try:
                            ...
                            if self._has_setup_skill():
                                _stt_msg += "\n\nFor full setup instructions, type: `/skill rok-setup`"
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        if history and len(history) >= 4:
            ...
            _hyg_data = {}
            try:
                _hyg_cfg_path = _rok_home / "config.yaml"
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        # Detect profile name from ROK_HOME path
        # Profile paths look like: ~/.rok/profiles/<name>
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            (_rok_home / ".restart_notify.json").write_text(
        ...
        return "♻ Restarting gateway. If you aren't notified within 60 seconds, restart from the console with `rok gateway restart`."
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return "No personalities configured in `~/.rok/config.yaml`"
        ...
        try:
            ...
            if "pynacl" in err_lower or "nacl" in err_lower or "davey" in err_lower:
                return (
                    ...
                    "Install or reinstall Rok with the messaging extra, e.g. "
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        def _collect_and_upload():
            ...
            lines.append("\nShare these links with the Rok team for support.")
            return "\n".join(lines)
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        if platform not in self._UPDATE_ALLOWED_PLATFORMS:
            return "✗ /update is only available from messaging platforms. Run `rok update` from the terminal."
        ...
        if not rok_cmd:
            return (
                "✗ Could not locate the `rok` command. "
                "Rok is running, but the update command could not find the "
                "executable on PATH or via the current Python interpreter. "
                "Try running `rok update` manually in your terminal."
            )
        ...
        self._schedule_update_notification_watch()
        return "⚕ Starting Rok update… I'll stream progress here."
    ...
    async def _watch_update_progress(
        ...
        while loop.time() < deadline:
            ...
            if exit_code_path.exists():
                ...
                try:
                    ...
                    if exit_code == 0:
                        await adapter.send(chat_id, "✅ Rok update finished.")
                    else:
                        await adapter.send(chat_id, "❌ Rok update failed (exit code {}).".format(exit_code))
        ...
        if not exit_code_path.exists():
            ...
            await _flush_buffer()
            try:
                await adapter.send(chat_id, "❌ Rok update timed out after 30 minutes.")
            except Exception:
                pass
            for p in (pending_path, claimed_path, output_path,
                      exit_code_path, prompt_path):
    ...
    async def _enrich_message_with_transcription(
        ...
        if not getattr(self.config, "stt_enabled", True):
            ...
            if self._has_setup_skill():
                disabled_note += (
                    " You have a skill called rok-setup that can help "
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if (
                        ...
                        if self._has_setup_skill():
                            _no_stt_note += (
                                " You have a skill called rok-setup "
    ...
    async def _run_agent(
        ...
        progress_mode = (
            _resolved_tp
            or os.getenv("ROK_TOOL_PROGRESS_MODE")
            or "all"
        )
        ...
        # ROK_NOTIFY_INTERVAL env var.  Default 600s (10 min).
        # 0 = disable notifications.
        _NOTIFY_INTERVAL_RAW = float(os.getenv("ROK_NOTIFY_INTERVAL", 600))
        ...
        try:
            ...
            # ROK_TIMEOUT env var (env var takes precedence).
            # Default 1800s (30 min inactivity).  0 = unlimited.
            _agent_timeout_raw = float(os.getenv("ROK_TIMEOUT", 1800))
            _agent_timeout = _agent_timeout_raw if _agent_timeout_raw > 0 else None
            _agent_warning_raw = float(os.getenv("ROK_TIMEOUT_WARNING", 900))
