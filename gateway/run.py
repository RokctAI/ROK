...
def _check_unavailable_skill(command_name: str) -> str | None:
    ...
    try:
        ...
        from rok_constants import get_rok_home, get_optional_skills_dir
...
class GatewayRunner:
    ...
    def _load_reasoning_config() -> dict | None:
        ...
        ROK_REASONING_EFFORT as a fallback. Valid: "xhigh", "high",
        ...
        try:
            ...
            pass
        if not effort:
            effort = os.getenv("ROK_REASONING_EFFORT", "")
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        _STALE_TTL = float(os.getenv("ROK_AGENT_TIMEOUT", 600)) + 60  # timeout + 1 min grace
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        # Spawn `rok update` detached so it survives gateway restart.
        # Use setsid for portable session detach (works under system services
        # where systemd-run --user fails due to missing D-Bus session).
        rok_cmd_str = " ".join(shlex.quote(part) for part in rok_cmd)
        update_cmd = (
            f"{rok_cmd_str} update > {shlex.quote(str(output_path))} 2>&1; "
        ...
        self._schedule_update_notification_watch()
        return "⚕ Starting Rok update… I'll notify you when it's done."
    ...
    async def _watch_for_update_completion(
        ...
        """Wait for ``rok update`` to finish, then send its notification."""
        pending_path = _rok_home / ".update_pending.json"
    ...
    def _set_session_env(self, context: SessionContext) -> None:
        ...
        os.environ["ROK_SESSION_PLATFORM"] = context.source.platform.value
        os.environ["ROK_SESSION_CHAT_ID"] = context.source.chat_id
        if context.source.chat_name:
            os.environ["ROK_SESSION_CHAT_NAME"] = context.source.chat_name
        if context.source.thread_id:
            os.environ["ROK_SESSION_THREAD_ID"] = str(context.source.thread_id)
    
    def _clear_session_env(self) -> None:
        """Clear session environment variables."""
        for var in ["ROK_SESSION_PLATFORM", "ROK_SESSION_CHAT_ID", "ROK_SESSION_CHAT_NAME", "ROK_SESSION_THREAD_ID"]:
    ...
    async def _run_agent(
        ...
        try:
            ...
            _agent_timeout = float(os.getenv("ROK_AGENT_TIMEOUT", 600))
            loop = asyncio.get_event_loop()
            try:
