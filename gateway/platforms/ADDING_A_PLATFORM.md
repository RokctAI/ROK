...
There are two ways to add a platform to the Rok gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.rok/plugins/` (or under `plugins/platforms/`
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `rok gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `rok_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Rok core.
...
And add it to the `rok-gateway` composite:

```python
"rok-gateway": {
    "includes": [..., "rok-your-platform"]
}
```
...
## 12. Status Display (`rok_cli/status.py`)
...
## 13. Gateway Setup Wizard (`rok_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ rok_cli/ toolsets.py \
  --include="*.py" -l | sort -u
