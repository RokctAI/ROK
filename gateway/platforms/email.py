"""
Email platform adapter for the Rok gateway.

Allows users to interact with Rok by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "rok-agent" "version" "{_rok_version}" '
            '"vendor" "Rokctai" '
            '"support-email" "noreply@rokctai.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Rok Agent")
        ...
        msg_id = f"<rok-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
