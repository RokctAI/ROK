...
| **Custom Endpoint** | `rok model` (saved in `config.yaml`) or `OPENAI_BASE_URL` + `OPENAI_API_KEY` in `~/.rok/.env` |
...
```bash
# z.ai / ZhipuAI GLM
rok chat --provider zai --model glm-4-plus
# Requires: GLM_API_KEY in ~/.rok/.env

# Kimi / Moonshot AI
rok chat --provider kimi-coding --model moonshot-v1-auto
...
`OPENAI_BASE_URL` and `LLM_MODEL` in `.env` are **deprecated**. The CLI ignores `LLM_MODEL` entirely (only the gateway reads it). Use `rok model` or edit `config.yaml` directly — both persist correctly across restarts and Docker containers.
:::
...
**Possible causes:**
1. **Low `max_tokens` on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Rok with `model.max_tokens` in config.yaml.
2. **Context exhaustion** — The model filled its context window. Increase context length or enable [context compression](/docs/user-guide/configuration#context-compression) in Rok.
