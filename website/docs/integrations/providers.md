...
| **Anthropic** | `rok model` (Claude Pro/Max via Claude Code auth, Anthropic API key, or manual setup-token) |
...
| **Alibaba Cloud** | `DASHSCOPE_API_KEY` in `~/.rok/.env` (provider: `alibaba`, aliases: `dashscope`, `qwen`) |
...
Configure a backup provider:model that Rok switches to automatically when your primary model fails (rate limits, server errors, auth failures):
...
Optional cheap-vs-strong routing lets Rok keep your main model for complex work while sending very short/simple turns to a cheaper model.
...
How it works:
- If a turn is short, single-line, and does not look code/tool/debug heavy, Rok may route it to `cheap_model`
- If the turn looks complex, Rok stays on your primary model/provider
- If the cheap route cannot be resolved cleanly, Rok falls back to the primary model automatically
