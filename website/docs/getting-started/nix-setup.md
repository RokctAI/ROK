...
description: "Install and deploy Rok Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---

# Nix & NixOS Setup

Rok Agent ships a Nix flake with three levels of integration:
...
**For non-NixOS users**, this only changes the install step. Everything after (`rok setup`, `rok gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage rok the same way you manage any other NixOS service.
:::
...
That's it. `nixos-rebuild switch` creates the `rok` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o rok /dev/stdin /var/lib/rok/env
```
...
rok version
rok config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Rok defaults to OpenRouter.
:::
...
| Add API keys | `environmentFiles` | `[ config.sops.secrets."rok-env".path ]` |
| Give the agent a personality | `${services.rok-agent.stateDir}/.rok/SOUL.md` | manage the file directly |
...
{
  sops = {
    defaultSopsFile = ./secrets/rok.yaml;
...
```yaml
# secrets/rok.yaml (encrypted with sops)
rok-env: |
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Rok looks for specific filenames by convention:

- **`USER.md`** — context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

The agent identity file is separate: Rok loads its primary `SOUL.md` from `$ROK_HOME/SOUL.md`, which in the NixOS module is `${services.rok-agent.stateDir}/.rok/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
:::tip
Environment variables in `env` values are resolved from `$ROK_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
Set `auth = "oauth"` for servers using OAuth 2.1. Rok implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
Tokens are stored in `$ROK_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.

<details>
<summary><strong>Initial OAuth authorization on headless servers</strong></summary>

The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Rok prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u rok` (native):
...
# Native mode
sudo -u rok ROK_HOME=/var/lib/rok/.rok \
  rok mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
```bash
rok mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.rok/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/rok/.rok/mcp-tokens/
# Ensure: chown rok:rok, chmod 0600
```
...
When rok runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `rok setup` | Config is declarative — edit `settings` in your Nix config |
| `rok config edit` | Config is generated from `settings` |
| `rok config set <key> <value>` | Config is generated from `settings` |
| `rok gateway install` | The systemd service is managed by NixOS |
| `rok gateway uninstall` | The systemd service is managed by NixOS |
...
When container mode is enabled, rok runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the rok package itself do **not** trigger recreation.

:::warning Writable layer loss
When the identity hash changes (image upgrade, new volumes, new container options), the container is destroyed and recreated from a fresh pull of `container.image`. Any `apt install`, `pip install`, or `npm install` packages in the writable layer are lost. State in `/data` and `/home/rok` is preserved (these are bind mounts).

If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/rok-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current rok package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `rok plugins install` needed.

### Directory Plugins (`extraPlugins`)

For plugins that are just a source tree with `plugin.yaml` + `__init__.py` (e.g., [rok-lcm](https://github.com/stephenschoettler/rok-lcm)):

```nix
services.rok-agent.extraPlugins = [
...
Plugins are symlinked into `$ROK_HOME/plugins/` at activation time. Rok discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.

### Entry-Point Plugins (`extraPythonPackages`)

For pip-packaged plugins that register via `[project.entry-points."rok_agent.plugins"]` (e.g., [rtk-rok](https://github.com/ogallotti/rtk-rok)):

```nix
services.rok-agent.extraPythonPackages = [
...
The package's `site-packages` is added to PYTHONPATH in the rok wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras already declared in rok-agent's `pyproject.toml` (e.g., memory providers like `hindsight` or `honcho`), use `extraDependencyGroups` to include them in the sealed venv at build time:
...
```nix
{
  inputs.rok-agent.url = "github:NousResearch/hermes-agent";
  outputs = { rok-agent, nixpkgs, ... }: {
    nixpkgs.overlays = [ rok-agent.overlays.default ];
    # Then:
    #   pkgs.rok-agent.override { extraPythonPackages = [...]; }
    #   pkgs.rok-agent.override { extraDependencyGroups = [ "hindsight" ]; }
  };
}
```
...
```nix
services.rok-agent.settings.plugins.enabled = [
  "rok-lcm"
  "rtk-rewrite"
];
```
...
:::note
A build-time collision check prevents plugin packages from shadowing core rok dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
nix build .#checks.x86_64-linux.managed-guard       # ROK_MANAGED blocks mutation
...
| `bundled-skills` | Skills directory exists, contains SKILL.md files, `ROK_BUNDLED_SKILLS` is set in wrapper |
...
| `stateDir` | `str` | `"/var/lib/rok"` | State directory (`ROK_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory (`MESSAGING_CWD`) |
| `addToSystemPackages` | `bool` | `false` | Add `rok` CLI to system PATH and set `ROK_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$ROK_HOME/.env` at activation time |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `rok gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the rok user's per-user profile so terminal commands, skills, and cron jobs all see them |
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$ROK_HOME/plugins/`. Each must contain `plugin.yaml` |
...
```
/var/lib/rok/                     # stateDir (owned by rok:rok, 0750)
├── .rok/                         # ROK_HOME
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Rok binary + all Nix deps |
| `/data` | `/var/lib/rok` | `rw` | All state, config, workspace |
| `/home/rok` | `${stateDir}/home` | `rw` | Persistent agent home — `pip install --user`, tool caches |
...
cd /etc/nixos && nix flake update rok-agent
...
| `no container with name or ID "rok-agent"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
...
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart rok-agent` |
