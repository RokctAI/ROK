...
description: "How to update Rok Agent to the latest version or uninstall it"
---
...
rok config check
rok config migrate   # Interactively add any missing options
```
...
:::warning
Rolling back may cause config incompatibilities if new options were added. Run `rok config check` after rolling back and remove any unrecognized options from `config.yaml` if you encounter errors.
:::
...
```bash
rok gateway stop
# Linux: systemctl --user disable rok-gateway
# macOS: launchctl remove ai.rok.gateway
```
:::
