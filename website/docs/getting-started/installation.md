...
Get Rok Agent up and running in under two minutes with the one-line installer, or follow the manual steps for full control.
...
:::tip
You do **not** need to activate the venv to use `rok`. The entry point has a hardcoded shebang pointing to the venv Python, so it works globally once symlinked.
:::
...
mkdir -p ~/.rok/{cron,sessions,logs,memories,skills,pairing,hooks,image_cache,audio_cache,whatsapp/session}
...
Open `~/.rok/.env` and add at minimum an LLM provider key:
...
```bash
rok config set OPENROUTER_API_KEY sk-or-v1-your-key-here
```

### Step 8: Add `rok` to Your PATH
...
```bash
rok model       # Select your LLM provider and model
```

### Step 10: Verify the Installation

```bash
rok version    # Check that the command is available
rok doctor     # Run diagnostics to verify everything is working
rok status     # Check your configuration
rok chat -q "Hello! What tools do you have available?"
```
