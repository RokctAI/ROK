...
description: "Run Rok Agent directly on an Android phone with Termux"
---

# Rok on Android with Termux

This is the tested path for running Rok Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Rok from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Rok now ships a Termux-aware installer path:
...
- links `rok` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `rok` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/rok" "$PREFIX/bin/rok"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `rok` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.rok/.env`.
...
### `rok doctor` says ripgrep or Node is missing
