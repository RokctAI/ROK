...
This tutorial walks you through setting up a Telegram bot powered by Rok Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Rok Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.rok/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Rok itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Rok Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_rok_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Rok Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u rok-gateway -f
...
rok gateway start
rok gateway stop
tail -f ~/.rok/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `rok gateway install` to update the plist.
:::
...
```bash
rok gateway stop && rok gateway start
```
...
rok pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.rok/.env`:
...
Control how much detail the bot shows when using tools. In `~/.rok/config.yaml`:
...
Customize how the bot communicates by editing `~/.rok/SOUL.md`:

For a full guide, see [Use SOUL.md with Rok](/docs/guides/use-soul-with-rok).
...
```bash
# From the CLI
rok cron list          # View all scheduled jobs
rok cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u rok-gateway` (Linux) or `~/.rok/logs/gateway.log` (macOS) |
| Cron job output | `~/.rok/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.rok/cron/jobs.json` |
| Pairing data | `~/.rok/pairing/` |
| Session history | `~/.rok/sessions/` |
