...
This guide shows how to actually use MCP with Rok Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Rok tool
- you want Rok to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Rok to internal APIs, databases, or company systems without modifying Rok core

Do not use MCP when:
- a built-in Rok tool already solves the job well
...
- Rok remains the agent
- MCP servers contribute tools
- Rok discovers those tools at startup or reload time
...
If you installed Rok with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Rok banner/status should show MCP integration when configured
- ask Rok what tools it has available
...
## WSL2: bridge Rok in WSL to Windows Chrome

This is the practical setup when:

- Rok runs inside WSL2
...
In this setup, Rok does **not** connect to Chrome directly. Instead:

- Rok runs in WSL
- Rok starts a local stdio MCP server
...
```text
Rok (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Rok stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Rok core browser transport
...
```bash
rok mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
rok mcp test chrome-devtools-win
```

Then start a fresh Rok session or run:
...
Once loaded, Rok can use the MCP-prefixed browser tools directly. For example:
...
If Rok runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Rok from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Rok from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Rok:
...
So Rok will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Rok to reason over a bounded workspace.
...
Now Rok can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Rok core.
...
Because Rok now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
