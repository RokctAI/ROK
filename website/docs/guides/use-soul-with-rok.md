...
# Use SOUL.md with Rok

`SOUL.md` is the **primary identity** for your Rok instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Rok to feel like the same assistant every time you talk to it — or if you want to replace the Rok persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Rok should be
- what Rok should avoid stylistically
- how Rok should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Rok is and how Rok speaks
...
Rok now uses only the global SOUL file for the current instance:

```text
~/.rok/SOUL.md
```

If you run Rok with a custom home directory, it becomes:
...
Rok automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Rok does not overwrite it
- if the file exists but is empty, Rok adds nothing from it to the prompt

## How Rok uses it

When Rok starts a session, it reads `SOUL.md` from `ROK_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Rok feels.
...
Rok already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Rok should sound.

# Avoid
What Rok should not do.

# Defaults
How Rok should behave when ambiguity appears.
```
...
Then restart Rok or start a new session.
...
### I edited SOUL.md but Rok still sounds the same

Check:
- you edited `~/.rok/SOUL.md` or `$ROK_HOME/SOUL.md`
...
### Rok is ignoring parts of my SOUL.md
