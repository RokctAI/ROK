...
```bash
rok teams-pipeline token-health
rok teams-pipeline token-health --force-refresh
```
...
```bash
rok teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
rok teams-pipeline maintain-subscriptions
rok teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Rok cron (recommended if you already run the Rok gateway)

Rok ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.rok/scripts/`. First create the script:

```bash
mkdir -p ~/.rok/scripts
cat > ~/.rok/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec rok teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.rok/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
rok cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/rok-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Rok Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=rok
EnvironmentFile=/etc/rok/env
ExecStart=/usr/local/bin/rok teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/rok-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Rok Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now rok-teams-pipeline-maintain.timer
systemctl list-timers rok-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/rok teams-pipeline maintain-subscriptions >> /var/log/rok/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.rok/.env` at the top of a wrapper script that crontab calls.
...
```bash
rok teams-pipeline subscriptions   # should show expirationDateTime advanced
rok teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
rok teams-pipeline list
rok teams-pipeline list --status failed
rok teams-pipeline show <job-id>
```

### Replay a stored job

```bash
rok teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
rok teams-pipeline fetch --meeting-id <meeting-id>
rok teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `rok teams-pipeline maintain-subscriptions --dry-run`
- inspect `rok teams-pipeline list --status failed`
...
- run `rok teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `rok teams-pipeline run`
...
- [ ] `rok teams-pipeline validate` returns an OK snapshot
- [ ] `rok teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Rok cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
