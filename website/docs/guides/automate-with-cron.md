...
description: "Real-world automation patterns using Rok cron — monitoring, reports, pipelines, and multi-skill workflows"
---
...
```python title="~/.rok/scripts/watch-site.py"
import hashlib, json, os, urllib.request

URL = "https://example.com/pricing"
STATE_FILE = os.path.expanduser("~/.rok/scripts/.watch-site-state.json")

# Fetch current content
req = urllib.request.Request(URL, headers={"User-Agent": "Rok-Monitor/1.0"})
...
```bash
/cron add "every 1h" "If the script output says CHANGE DETECTED, summarize what changed on the page and why it might matter. If it says NO_CHANGE, respond with just [SILENT]." --script ~/.rok/scripts/watch-site.py --name "Pricing monitor" --deliver telegram
```
...
```python title="~/.rok/scripts/collect-prices.py"
import json, os, urllib.request
from datetime import datetime

DATA_DIR = os.path.expanduser("~/.rok/data/prices")
...
If there's a significant move, explain what happened." \
  --script ~/.rok/scripts/collect-prices.py \
  --name "Price tracker" \
  --deliver telegram
```
