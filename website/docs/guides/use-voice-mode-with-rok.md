...
- you want Rok sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Rok.
...
## Step 1: make sure normal Rok works first
...
Rok supports both local and cloud speech stacks.
...
If you choose NeuTTS in the setup wizard, Rok checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Rok transcribes and responds
...
- asking Rok to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Rok loop.

## Tuning CLI behavior

### Silence threshold

If Rok starts/stops too aggressively, tune:
...
Rok stays a normal chat bot, but can speak replies.
...
- you want Rok to function like a portable research or ops assistant
...
Rok joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Rok detects speech boundaries
- transcripts are posted in the associated text channel
- Rok responds in text and audio
