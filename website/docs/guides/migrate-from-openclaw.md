...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env`. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — those values must be added to Rok manually after migration.
...
3. **Verify API keys** — run `rok status` to check provider authentication.

4. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart rok-gateway`

5. **Check session policies** — verify `rok config get session_reset` matches your expectations.

6. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `rok whatsapp` to pair.
