...
description: "How Rok stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `rok_cli/cron.py` | CLI `rok cron` subcommands |
...
Jobs are stored in `~/.rok/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `rok cron` commands are run or during active CLI sessions.
...
```python
# ~/.rok/scripts/check_competitors.py
...
2. **Environment variable** — `ROK_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.rok/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `rok cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `rok cron` CLI provides direct job management:

```bash
rok cron list                    # Show all jobs
rok cron create                  # Interactive job creation (alias: add)
rok cron edit <job_id>           # Edit job configuration
rok cron pause <job_id>          # Pause a running job
rok cron resume <job_id>         # Resume a paused job
rok cron run <job_id>            # Trigger immediate execution
rok cron remove <job_id>         # Delete a job
```
