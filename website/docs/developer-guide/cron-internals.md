...
Rok cron support is implemented primarily in:
...
Cron jobs are stored in Rok-managed local state (`~/.rok/cron/jobs.json`) with atomic write semantics.
...
A cron job may attach multiple skills. At runtime, Rok loads those skills in order and then appends the job prompt as the task instruction.
...
Rok uses lock-based protections so overlapping scheduler ticks do not execute the same due-job batch twice.
