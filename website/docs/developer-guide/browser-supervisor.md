...
[PR #12550](https://github.com/RokctAI/rok-agent/pull/12550) proposed a
...
(`rok-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Rok `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `rok-acp`, `rok-api-server`, core toolsets (gated on CDP reachability)
...
- `rok_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
