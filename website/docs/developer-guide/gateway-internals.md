...
The messaging gateway is the long-running process that connects Rok to 14+ external messaging platforms through a unified architecture.
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.rok/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
