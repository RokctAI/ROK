...
The ACP adapter wraps Rok' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
rok acp / rok-acp / python -m acp_adapter
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'rok-agent[acp]==<version>' rok-acp`, pointed at the `rok-agent` PyPI release.
...
- `allow_once` -> Rok `once`
- `allow_always` -> Rok `always`
- reject options -> Rok `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Rok tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["rok-acp"])
...
Instead it reuses Rok' runtime resolver:

- `acp_adapter/auth.py`
- `rok_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Rok provider/credentials. It also always advertises a terminal setup auth method (`rok-setup`, args `--setup`) so first-run registry clients can open Rok' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.rok/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `rok-acp` toolset definition
- `rok_cli/main.py` — `rok acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `rok-acp` script
