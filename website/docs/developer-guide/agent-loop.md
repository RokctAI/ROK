...
Rok currently supports three API execution modes:
...
Rok wraps API requests so they can be interrupted from the CLI or gateway.
...
Rok uses two execution strategies:
...
Rok tracks a shared iteration budget across parent and subagents. It also injects budget pressure hints near the end of the available iteration window.
...
Before and during long runs, Rok may:
