...
Rok Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Rok/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
parser = get_parser("rok")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `rok`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"rok"`, `"qwen"`, `"mistral"`)
...
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which rok toolsets to enable |
...
| `tool_call_parser` | `str` | `"rok"` | Parser for Phase 2 raw output |
...
│   ├── rok_parser.py        # Rok/ChatML <tool_call> format
...
├── rok_swe_env/             # SWE-bench training environment
│
