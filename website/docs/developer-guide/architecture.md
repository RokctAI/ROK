...
sidebar_position: 1
title: "Architecture"
description: "Rok Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Rok Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ rok_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # Rokcli — interactive terminal UI (~10,000 lines)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── rok_state.py           # SQLite session/state database with FTS5
├── rok_constants.py       # ROK_HOME, profile-aware paths
...
│   ├── main.py               # Entry point — all `rok` subcommands (~6,000 lines)
...
│   ├── skills_config.py      # rok skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # rok tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # rok gateway start/stop
│
...
```text
User input → Rokcli.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .rok.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.rok/plugins/` (user), `.rok/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `rok plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-rok-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Rok as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`rok -p <name>`) gets its own ROK_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
