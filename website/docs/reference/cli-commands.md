...
| `--ignore-user-config` | Ignore `~/.rok/config.yaml` and fall back to built-in defaults. Credentials in `.env` are still loaded. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, memory, and preloaded skills. |
| `--tui` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI. Equivalent to `ROK_TUI=1`. |
...
| `rok fallback` | Manage fallback providers tried when the primary model errors. |
| `rok gateway` | Run or manage the messaging gateway service. |
| `rok proxy` | Local OpenAI-compatible proxy that attaches OAuth provider credentials. See [Subscription Proxy](../user-guide/features/subscription-proxy.md). |
| `rok lsp` | Manage Language Server Protocol integration (semantic diagnostics for write_file/patch). |
...
| `rok slack` | Slack helpers (currently: generate the app manifest with every command as a native slash). |
...
| `rok kanban` | Multi-profile collaboration board (tasks, links, dispatcher). |
| `rok webhook` | Manage dynamic webhook subscriptions for event-driven activation. |
| `rok hooks` | Inspect, approve, or remove shell-script hooks declared in `config.yaml`. |
...
| `rok checkpoints` | Inspect / prune / clear `~/.rok/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
...
| `rok curator` | Background skill maintenance — status, run, pause, pin. See [Curator](../user-guide/features/curator.md). |
| `rok memory` | Configure external memory provider. Plugin-specific subcommands (e.g. `rok honcho`) register automatically when their provider is active. |
...
| `rok computer-use` | Install or check the cua-driver backend (macOS Computer Use). |
...
| `rok completion` | Print shell completion scripts (bash/zsh/fish). |
| `rok version` | Show version information. |
| `rok update` | Pull latest code and reinstall dependencies (git installs), or check PyPI and `pip install --upgrade` (pip installs). `--check` previews without installing; `--backup` takes a pre-pull `ROK_HOME` snapshot. |
...
| `--ignore-user-config` | Ignore `~/.rok/config.yaml` and use built-in defaults. Credentials in `.env` are still loaded. Useful for isolated CI runs, reproducible bug reports, and third-party integrations. |
...
rok chat --ignore-user-config --ignore-rules -q "Repro without my personal setup"
```

### `rok -z <prompt>` — scripted one-shot

For programmatic callers (shell scripts, CI, cron, parent processes piping in a prompt), `rok -z` is the purest one-shot entry point: **single prompt in, final response text out, nothing else on stdout or stderr.** No banner, no spinner, no tool previews, no `Session:` line — just the agent's final reply as plain text.

```bash
rok -z "What's the capital of France?"
# → Paris.

# Parent scripts can cleanly capture the response:
answer=$(rok -z "summarize this" < /path/to/file.txt)
```

Per-run overrides (no mutation to `~/.rok/config.yaml`):

| Flag | Equivalent env var | Purpose |
|---|---|---|
| `-m` / `--model <model>` | `ROK_INFERENCE_MODEL` | Override the model for this run |
| `--provider <provider>` | `ROK_INFERENCE_PROVIDER` | Override the provider for this run |

```bash
rok -z "…" --provider openrouter --model openai/gpt-5.5
# or:
ROK_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 rok -z "…"
```

Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `rok chat -q` instead; `-z` is explicitly for "I only want the final answer".

## `rok model`

Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active Rok chat session.
...
:::warning rok model vs /model — know the difference
**`rok model`** (run from your terminal, outside any Rok session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active Rok chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your Rok session first (`Ctrl+C` or `/quit`), then run `rok model` from your terminal prompt.
:::
...
If you've only configured OpenRouter, `/model` will only show OpenRouter models. To add another provider (Anthropic, DeepSeek, Copilot, etc.), exit your session and run `rok model` from the terminal.
:::
...
| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `ROK_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `rok update`. |
...
```bash
rok setup [model|tts|terminal|gateway|tools|agent] [--non-interactive] [--reset] [--quick] [--reconfigure]
```
...
| `--reconfigure` | Backwards-compat alias — bare `rok setup` on an existing install now does this by default. |
...
```bash
rok slack manifest              # print manifest to stdout
rok slack manifest --write      # write to ~/.rok/slack-manifest.json
rok slack manifest --slashes-only  # just the features.slash_commands array
```
...
|------|---------|---------|
| `--write [PATH]` | stdout | Write to a file instead of stdout. Bare `--write` writes `$ROK_HOME/slack-manifest.json`. |
| `--name NAME` | `Rok` | Bot display name in Slack. |
| `--description DESC` | default blurb | Bot description shown in the Slack app directory. |
| `--slashes-only` | off | Emit only `features.slash_commands` for merging into a manually-maintained manifest. |

Run `rok slack manifest --write` again after `rok update` to pick
up any new commands.
...
rok auth status anthropic                             # Show auth status for a provider
rok auth logout anthropic                             # Log out and clear stored auth state
rok auth spotify                                      # Authenticate Rok with Spotify via PKCE
```
...
```bash
rok kanban [--board <slug>] <action> [options]
```

Multi-profile, multi-project collaboration board. Each install can host many boards (one per project, repo, or domain); each board is a standalone queue with its own SQLite DB and dispatcher scope. New installs start with one board called `default`, whose DB is `~/.rok/kanban.db` for back-compat; additional boards live at `~/.rok/kanban/boards/<slug>/kanban.db`. The gateway-embedded dispatcher sweeps every board per tick.

**Global flags (apply to every action below):**

| Flag | Purpose |
|------|---------|
| `--board <slug>` | Operate on a specific board. Defaults to the current board (set via `rok kanban boards switch`, the `ROK_KANBAN_BOARD` env var, or `default`). |

**This is the human / scripting surface.** Agent workers spawned by the dispatcher drive the board through a dedicated `kanban_*` [toolset](/docs/user-guide/features/kanban#how-workers-interact-with-the-board) (`kanban_show`, `kanban_complete`, `kanban_block`, `kanban_create`, `kanban_link`, `kanban_comment`, `kanban_heartbeat`) instead of shelling to `rok kanban`. Workers have `ROK_KANBAN_BOARD` pinned in their env so they physically cannot see other boards.
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.rok/kanban/current`). |
...
rok kanban boards create atm10-server --name "ATM10 Server" --icon 🎮
rok kanban --board atm10-server create "Restart server" --assignee ops

# Switch the active board for subsequent calls.
rok kanban boards switch atm10-server
rok kanban list                  # shows atm10-server tasks

# Archive a board (recoverable) or hard-delete it.
rok kanban boards rm atm10-server
rok kanban boards rm atm10-server --delete
```

Board resolution order (highest precedence first): `--board <slug>` flag → `ROK_KANBAN_BOARD` env var → `~/.rok/kanban/current` file → `default`.

All actions are also available as a slash command in the gateway (`/kanban …`), with the same argument surface — including `boards` subcommands and the `--board` flag.

For the full design — comparison with Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise, eight collaboration patterns, four user stories, concurrency correctness proof — see `docs/rok-kanban-v1-spec.pdf` in the repository or the [Kanban user guide](/docs/user-guide/features/kanban).
...
Create a zip archive of your Rok configuration, skills, sessions, and data. The backup excludes the rok-agent codebase itself.
...
- The `rok-agent` code itself (this is a user-data backup, not a repo snapshot).
...
```bash
rok checkpoints [COMMAND]
```

Inspect and manage the shadow git store at `~/.rok/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.

| Subcommand | Description |
|------------|-------------|
| `status` (default) | Show total size, project count, and per-project breakdown. Bare `rok checkpoints` is equivalent. |
...
```bash
rok checkpoints                                  # status overview
rok checkpoints prune --retention-days 3         # aggressive cleanup
rok checkpoints prune --max-size-mb 200          # tighten size cap once
rok checkpoints clear-legacy -f                  # drop v1 archive dirs
rok checkpoints clear -f                         # wipe everything
```
...
Restore a previously created Rok backup into your Rok home directory. All files in the archive overwrite existing files in your Rok home; `--force` only skips the confirmation prompt that fires when the target already has a Rok installation.
...
### Examples
```bash
rok import ~/rok-backup-20260423.zip           # Prompts before overwriting existing config
rok import ~/rok-backup-20260423.zip --force   # Overwrite without prompting
```
...
rok skills install https://sharethis.chat/SKILL.md                     # Direct URL (single-file SKILL.md)
rok skills install https://example.com/SKILL.md --name my-skill        # Override name when frontmatter has none
rok skills check
rok skills update
rok skills config
rok skills reset google-workspace
rok skills reset google-workspace --restore --yes
```
...
| `backup` | Take a manual tar.gz snapshot of `~/.rok/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.rok/skills/` from a snapshot (defaults to newest) |
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `rok update`. Use `rok curator run --dry-run` to preview before that happens.
...
| `add` | Pick a provider + model (same picker as `rok model`) and append to the chain |
...
```bash
rok hooks <subcommand>
```

Inspect shell-script hooks declared in `~/.rok/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.rok/shell-hooks-allowlist.json`.
...
When an external memory provider is active, it may register its own top-level `rok <provider>` command for provider-specific management (e.g. `rok honcho` when Honcho is active). Inactive providers do not expose their subcommands. Run `rok --help` to see what's currently wired in.
:::
...
`rok computer-use install` is the stable entry point for installing the
[cua-driver](https://github.com/trycua/cua) binary used by the
`computer_use` toolset. It runs the same upstream installer that
`rok tools` invokes when you first enable Computer Use, so it's safe
to use for re-running the install if the toolset toggle didn't trigger
it (for example, on returning-user setups).

`rok update` automatically re-runs the upstream installer at the end
of the update if cua-driver is on PATH, so most users will not need to
call `--upgrade` manually. Use it when upstream ships a fix you want
right now without waiting for the next Rok update.
...
| `--overwrite` | Overwrite existing Rok files on conflicts (default: refuse to apply when the plan has conflicts). |
| `--migrate-secrets` | Include API keys in migration. Required even under `--preset full`. |
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.rok/` (by default a single restore-point archive is written to `~/.rok/backups/pre-migration-*.zip` before apply; restorable with `rok import`). |
...
rok claw migrate --preset full --migrate-secrets
...
Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. Requires `pip install rok-agent[web]` (FastAPI + Uvicorn). See [Web Dashboard](/docs/user-guide/features/web-dashboard) for full documentation.
...
rok profile install github.com/user/my-distro --alias
rok profile update work
...
```bash
rok update [--check] [--backup] [--restart-gateway]
```

Pulls the latest `rok-agent` code and reinstalls dependencies in your venv, then re-runs the post-install hooks (MCP servers, skills sync, completion install). Safe to run on a live install.

**pip installs:** `rok update` detects pip-based installations automatically — it queries PyPI for the latest release and runs `pip install --upgrade rok-agent` instead of `git pull`. PyPI releases track tagged versions (major/minor releases), not every commit on `main`. Use `--check` to see if a newer PyPI release is available without installing.
...
| `--backup` | Create a labeled pre-update snapshot of `ROK_HOME` (config, auth, sessions, skills, pairing data) before pulling. Default is **off** — the previous always-backup behavior was adding minutes to every update on large homes. Flip it on permanently via `update.backup: true` in `config.yaml`. |
...
- **Pairing data snapshot.** Even when `--backup` is off, `rok update` takes a lightweight snapshot of `~/.rok/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `rok backup restore --state pre-update` if a pull rewrites a file you were editing.
- **Legacy `rok.service` warning.** If Rok detects a pre-rename `rok.service` systemd unit (instead of the current `rok-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
