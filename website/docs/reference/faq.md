...
description: "Frequently asked questions and solutions to common issues with Rok Agent"
---
...
### What LLM providers work with Rok?

Rok Agent works with any OpenAI-compatible API. Supported providers include:
...
Set your provider with `rok model` or by editing `~/.rok/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Rok Agent requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Rok from inside it. The standard install command works perfectly in WSL2:
...
Yes. Run `rok model`, select **Custom endpoint**, and enter your server's URL:
...
Rok persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Rok Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
#### `rok: command not found` after installation
...
**Cause:** Rok requires Python 3.11 or newer.
...
# Or set directly
rok config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.rok/.env` for conflicting entries.
:::
...
- Using `rok chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Rok detected the wrong context length for your model.
...
**Cause:** Rok detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Rok never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `rok chat`
...
cat ~/.rok/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `rok gateway setup`
- Check gateway logs: `cat ~/.rok/logs/gateway.log | tail -50`
...
Configure in `~/.rok/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
**Solution:** The gateway captures your shell PATH when you run `rok gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
rok gateway install    # Re-snapshots your current PATH
rok gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.rok.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `rok chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `rok chat -t "terminal"`
...
Verify your `~/.rok/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
rok config show | grep -A 12 mcp_servers

# Restart Rok or reload MCP after config changes
rok chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with Rok](/docs/guides/use-mcp-with-rok)
...
:::warning
If an MCP server crashes mid-request, Rok will report a timeout. Check the server's own logs (not just Rok logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `rok profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `rok update`?
...
There is no hard limit. Each profile is just a directory under `~/.rok/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Rok can route subagents to a different model automatically. Set this in `~/.rok/config.yaml`:
...
Now when you tell Rok "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Rok do this?

**Current limitation:** Rok profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Rok reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `rok skills config` settings don't seem to take effect.

**Solution:** Use `rok skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`rok gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Rok creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Rok to another machine
...
1. Install Rok Agent on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/NousResearch/hermes-agent/main/scripts/install.sh | bash
   ```
...
**Scenario:** After running the Rok installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Rok-specific issue — it's a shell config permissions problem.
