...
| `ROK_MODEL` | Preferred model name (checked before `LLM_MODEL`, used by gateway) |
...
| Variable | Description |
|----------|-------------|
| `ROK_INFERENCE_PROVIDER` | Override provider selection: `auto`, `openrouter`, `nous`, `openai-codex`, `copilot`, `copilot-acp`, `anthropic`, `huggingface`, `zai`, `kimi-coding`, `minimax`, `minimax-cn`, `kilocode`, `alibaba`, `deepseek`, `opencode-zen`, `opencode-go`, `ai-gateway` (default: `auto`) |
...
| `ROK_API_TIMEOUT` | LLM API call timeout in seconds (default: `900`) |
