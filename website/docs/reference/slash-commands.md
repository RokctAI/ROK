...
User-defined quick commands from `quick_commands` in `~/.rok/config.yaml` are also available as slash commands. These are resolved at dispatch time, not shown in the built-in autocomplete/help tables.
