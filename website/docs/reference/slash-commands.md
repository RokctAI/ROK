...
| `/snapshot` (alias: `/snap`) | Create or restore state snapshots of Rok config/state (usage: /snapshot [create\|restore \<id\>\|prune]) |
...
| `/plan [request]` | Load the bundled `plan` skill to write a markdown plan instead of executing the work. Plans are saved under `.rok/plans/` relative to the active workspace/backend working directory. |
...
User-defined quick commands map a short alias to a longer prompt. Configure them in `~/.rok/config.yaml`:
...
| `/snapshot [create\|restore <id>\|prune]` (alias: `/snap`) | Create or restore state snapshots of Rok config/state. |
