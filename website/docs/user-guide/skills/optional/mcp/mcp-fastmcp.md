...
| | |
|---|---|
| Source | Optional — install with `rok skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Rok or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Rok. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.rok/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.rok/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Rok integration, either:

- configure the server in `~/.rok/config.yaml` using the `native-mcp` skill, or
...
### Rok cannot see the deployed server

The server-building part may be correct while the Rok config is not. Load the `native-mcp` skill and configure the server in `~/.rok/config.yaml`, then restart Rok.
