...
| | |
|---|---|
| Source | Optional — install with `rok skills install official/security/1password` |
...
- `tmux` available for stable authenticated sessions during Rok terminal calls (desktop app flow only)
...
### Service Account (recommended for Rok)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.rok/.env` (the skill will prompt for this on first load).
...
## Rok Execution Pattern (desktop app flow)

Rok terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/rok-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/rok-op.sock"
