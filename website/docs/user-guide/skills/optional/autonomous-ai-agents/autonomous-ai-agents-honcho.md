...
description: "Configure and use Honcho memory with Rok -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Rok -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `rok skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Rok

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Rok profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/rok#running-honcho-locally-with-rok

### Verify

```bash
rok honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Rok profile's AI peer
...
Honcho models conversations as interactions between **peers**. Rok creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Rok instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Rok run |
| `global` | Single session across all directories |

Manual override: `rok honcho map my-project-name`
...
Each Rok profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
rok profile create coder --clone
# creates host block rok.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `rok.coder` host block in `honcho.json`
...
```bash
rok honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Rok when Honcho memory is active.
...
Config file: `$ROK_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `rok honcho setup`. Ensure `memory.provider: honcho` is in `~/.rok/config.yaml`.

### Memory not persisting across sessions
Check `rok honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `rok profile create <name> --clone`. For existing profiles: `rok honcho sync`.
...
| Command | Description |
|---------|-------------|
| `rok honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `rok honcho status` | Show resolved config, connection test, peer info for active profile |
| `rok honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `rok honcho disable` | Disable Honcho for the active profile |
| `rok honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `rok honcho peers` | Show peer identities across all profiles |
| `rok honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `rok honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `rok honcho sessions` | List known directory-to-session-name mappings |
| `rok honcho map <name>` | Map current working directory to a Honcho session name |
| `rok honcho identity` | Seed AI peer identity or show both peer representations |
| `rok honcho sync` | Create host blocks for all Rok profiles that don't have one yet |
| `rok honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Rok + Honcho |
| `rok memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `rok memory status` | Show active memory provider and config |
| `rok memory off` | Disable external memory provider |
