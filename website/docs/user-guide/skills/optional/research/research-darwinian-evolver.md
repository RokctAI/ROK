...
| | |
|---|---|
| Source | Optional — install with `rok skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Rok Agent |
| License | MIT |
...
import upstream classes into Rok itself.
...
```bash
mkdir -p ~/.rok/cache/darwinian-evolver && cd ~/.rok/cache/darwinian-evolver
...
```bash
cd ~/.rok/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.rok/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.rok/skills/research/darwinian-evolver
DE_DIR=~/.rok/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Rok core.
   Custom driver scripts under `~/.rok/skills/...` are user-side and fine.
