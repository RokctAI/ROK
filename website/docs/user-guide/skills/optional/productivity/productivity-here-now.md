...
| | |
|---|---|
| Source | Optional — install with `rok skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ROK_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ROK_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ROK_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client rok
```
...
```bash
PUBLISH="${ROK_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client rok
```
...
```bash
DRIVE="${ROK_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `rok`)    |
