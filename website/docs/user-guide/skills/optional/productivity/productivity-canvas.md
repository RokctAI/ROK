...
| | |
|---|---|
| Source | Optional — install with `rok skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Rok Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.rok/.env`:
...
```bash
CANVAS="python $ROK_HOME/skills/productivity/canvas/scripts/canvas_api.py"
