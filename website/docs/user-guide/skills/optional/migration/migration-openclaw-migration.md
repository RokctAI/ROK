---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Rok Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Rok Agent"
---
...
Migrate a user's OpenClaw customization footprint into Rok Agent. Imports Rok-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `rok skills install official/migration/openclaw-migration` |
...
| Related skills | [`rok-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-rok-agent) |
...
Use this skill when a user wants to move their OpenClaw setup into Rok Agent with minimal manual cleanup.
...
```bash
rok claw migrate              # Full interactive migration
...
rok claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `rok setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_rok.py` to:

- import `SOUL.md` into the Rok home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Rok memory entries
- merge OpenClaw command approval patterns into Rok `command_allowlist`
- migrate Rok-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.rok/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Rok workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.rok/tts/`
- archive non-secret docs that do not have a direct Rok destination
...
- `scripts/openclaw_to_rok.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.rok/skills/migration/openclaw-migration/scripts/openclaw_to_rok.py`

Do not guess a shorter path like `~/.rok/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.rok/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Rok-compatible secrets, currently:
...
Rok CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Rok. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Rok workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Rok skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.rok/skills/migration/openclaw-migration/scripts/openclaw_to_rok.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.rok/skills/migration/openclaw-migration/scripts/openclaw_to_rok.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.rok/skills/migration/openclaw-migration/scripts/openclaw_to_rok.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.rok/skills/migration/openclaw-migration/scripts/openclaw_to_rok.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.rok/skills/migration/openclaw-migration/scripts/openclaw_to_rok.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.rok/skills/migration/openclaw-migration/scripts/openclaw_to_rok.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Rok unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Rok targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Rok destination. Unsupported auth blobs must still be reported as skipped.
...
- Rok persona state imported
- Rok memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.rok/skills/openclaw-imports/`
