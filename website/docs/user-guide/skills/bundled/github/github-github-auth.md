...
- Give it a title like "rok-agent-&lt;machine-name>"
...
elif [ -f ~/.rok/.env ] && grep -q "^GITHUB_TOKEN=" ~/.rok/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.rok/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
