...
Connect Rok to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Rok supports two WeCom integration modes:
...
3. Rok decrypts the message, queues it for the agent
...
(Use `rok gateway start` only after `rok gateway install` has registered the systemd/launchd service.)
