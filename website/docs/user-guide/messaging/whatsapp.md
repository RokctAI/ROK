...
The Baileys bridge saves its session under `~/.rok/whatsapp/session`. This means:
...
| **Session not persisting** | Check that `~/.rok/whatsapp/session` exists and is writable. If containerized, mount it as a persistent volume. |
...
- The `~/.rok/whatsapp/session` directory contains full session credentials — protect it like a password
- Set file permissions: `chmod 700 ~/.rok/whatsapp/session`
