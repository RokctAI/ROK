...
sidebar_position: 12
title: "Google Chat"
description: "Set up Rok Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Rok Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Rok
...
download the file. Save it somewhere only Rok can read (e.g.,
`~/.rok/google-chat-sa.json`, `chmod 600`).
...
- Topic ID: `rok-chat-events`
- Leave the defaults for everything else.

After creation, the topic's detail page has a **Subscriptions** tab. Create one:

- Subscription ID: `rok-chat-events-sub`
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a rok restart)
...
- Principal: `rok-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Rok calls
...
- **App name**: whatever you want users to see ("Rok" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/rok-chat-events`.
...
`ADDED_TO_SPACE` event that Rok uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Rok

Add the Google Chat section to `~/.rok/.env`:

```bash
# Required
GOOGLE_CHAT_PROJECT_ID=my-chat-bot-123
GOOGLE_CHAT_SUBSCRIPTION_NAME=projects/my-chat-bot-123/subscriptions/rok-chat-events-sub
GOOGLE_CHAT_SERVICE_ACCOUNT_JSON=/home/you/.rok/google-chat-sa.json
...
Install the dependencies the Google Chat adapter needs (no Rok extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Rok is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Rok detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Rok session.
...
3. Download the JSON. Move it onto the host that runs Rok.
4. On the host, register the client with Rok:
...
That writes `~/.rok/google_chat_user_client_secret.json`. This is shared
...
The token lands at `~/.rok/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.rok/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Rok isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `rok gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Rok will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.rok/google_chat_user_tokens/<sanitized_email>.json` (filesystem
