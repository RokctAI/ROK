...
| **Rooms** | Rok responds to all messages in rooms it has joined. Room invites are auto-accepted. |
| **Threads** | Rok supports Matrix threads (MSC3440). If you reply in a thread, Rok keeps the thread context isolated from the main room timeline. |
...
- Stores encryption keys in `~/.rok/matrix/store/`
...
:::warning
If you delete the `~/.rok/matrix/store/` directory, the bot loses its encryption keys. You'll need to verify the device again in your Matrix client. Back up this directory if you want to preserve encrypted sessions.
:::
