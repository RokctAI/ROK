...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Rok Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Rok to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.rok/.env`:
...
All standard Rok commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.rok/logs/gateway.log`
...
4. Consider enabling verbose logging: `ROK_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Rok automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
ROK_LOG_LEVEL=debug rok gateway
```
...
```bash
rok chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
