...
Open WebUI connects to Rok Agent's API server just like it would connect to OpenAI. Your agent handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.
...
With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and Rok Agent can preserve full tool call history across turns via `previous_response_id`.
...
2. Rok Agent creates an AIAgent instance with its full toolset
