...
Rok Agent automatically snapshots your project before **destructive operations** and lets you restore it with a single command. Checkpoints are **enabled by default** — there's zero cost when no file-mutating tools fire.

This safety net is powered by an internal **Checkpoint Manager** that keeps a separate shadow git repository under `~/.rok/checkpoints/` — your real project `.git` is never touched.
...
Checkpoints are enabled by default. Configure in `~/.rok/config.yaml`:
...
To keep checkpointing safe and fast, Rok applies several guardrails:
...
- A `ROK_WORKDIR` file pointing back to the original project root
