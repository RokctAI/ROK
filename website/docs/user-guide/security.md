...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` in `~/.rok/`. Skills declare these in frontmatter:
...
When loaded, Rok checks if these files exist in `~/.rok/` and registers them for mounting:
