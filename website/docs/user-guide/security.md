...
Rok Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Rok checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.rok/config.yaml`:
...
1. **CLI flag**: Start a session with `rok --yolo` or `rok chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `ROK_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `ROK_YOLO_MODE` environment variable which is checked before every command execution.
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Rok refuses to run them **regardless** of:
...
Configure the timeout in `~/.rok/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.rok/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.rok/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` rok/gateway | Self-termination prevention |
...
The `ROK_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.rok/config.yaml`:
...
:::tip
Use `rok config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Rok controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.rok/.env`:
...
For more flexible authorization, Rok includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `rok pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.rok/config.yaml`:
...
rok pairing approve telegram ABC12DEF

# Revoke a user's access
rok pairing revoke telegram 123456789

# Clear all pending codes
rok pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.rok/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Rok applies strict security hardening to every container.
...
Container resources are configurable in `~/.rok/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.rok/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `ROK_HOME`. Skills declare these in frontmatter:
...
When loaded, Rok checks if these files exist in the active profile's `ROK_HOME` and registers them for mounting:
...
Paths are relative to `~/.rok/`. Files are mounted to `/root/.rok/` inside the container.
...
| **terminal** (local) | Blocks explicit Rok infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Rok infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Rok integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Rok under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.rok/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.rok/logs/` for unauthorized access attempts
10. **Keep updated** — run `rok update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.rok/.env`:
...
TERMINAL_SSH_USER=rok
TERMINAL_SSH_KEY=~/.ssh/rok_agent_key
```
