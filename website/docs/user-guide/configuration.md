...
| `"nous"` | Force Nous Portal | `rok login` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `rok model` → Codex |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `rok model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. | Custom endpoint credentials + base URL |
...
`provider: "main"` follows the same custom endpoint Rok uses for normal chat. That endpoint can be set directly with `OPENAI_BASE_URL`, or saved once through `rok model` and persisted in `config.yaml`.
