...
Rok supports six terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox, a Daytona workspace, or a Singularity/Apptainer container.
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means Rok will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
**Requirements:** Docker Desktop or Docker Engine installed and running. Rok probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle).
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/rok` (HPC convention) → `~/.rok/sandboxes/singularity`.
...
```bash
rok config set skills.config.wiki.path ~/my-research-wiki
```
...
| API call (non-streaming) | 1800s | Unchanged | `ROK_API_TIMEOUT` |
...
Rok uses lightweight "auxiliary" models for side tasks like image analysis, web page summarization, and browser screenshot analysis. By default, these use **Gemini Flash** via auto-detection — you don't need to configure anything.
...
quick_commands:
  ...
  update:
    type: exec
    command: cd ~/.rok/rok && git pull && pip install -e .
  gpu:
    type: exec
...
The `web_search`, `web_extract`, and `web_crawl` tools support four backend providers. Configure the backend in `config.yaml` or via `rok tools`:
...
```yaml
checkpoints:
  enabled: true                  # Enable automatic checkpoints (also: rok --checkpoints)
