...
Rok Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
Rok stores session history so it can resume conversations, but it does not
...
Rok explicitly injects for that turn.
...
For example, if a user sends an image and asks Rok to make a meme from it,
Rok may inspect that image once with vision and run an image-processing
...
`rok sessions prune` only when you want to delete old ended sessions from
...
rok -c "my project"   # → resumes "my project #3"
```
...
rok --resume 20250305_091523_a1b2c3d4
rok -r 20250305_091523_a1b2c3d4

# Resume by title
rok --resume "refactoring auth"

# Or with the chat subcommand
rok chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `rok sessions list`.

### Conversation Recap on Resume

When you resume a session, Rok displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Rok session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.rok/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `rok -r "<title>"` from the shell) and pick up where the platform left off.
...
Rok automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `rok sessions list` or `rok sessions browse`.
...
```bash
rok sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Rok creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`rok -c "my project"`), it automatically picks the most recent session in the lineage.
...
Rok provides a full set of session management commands via `rok sessions`:
...
# Filter by platform
rok sessions list --source telegram

# Show more sessions
rok sessions list --limit 50
```
...
rok sessions export backup.jsonl

# Export sessions from a specific platform
rok sessions export telegram-history.jsonl --source telegram

# Export a single session
rok sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
rok sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
rok sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
rok sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
rok sessions prune --older-than 30

# Only prune sessions from a specific platform
rok sessions prune --source telegram --older-than 60

# Skip confirmation
rok sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`rok insights`](/docs/reference/cli-commands#rok-insights).
...
By default, Rok uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Rok in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.rok/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.rok/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.rok/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Rok process in the same `ROK_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.rok/config.yaml`:
...
rok sessions delete <session_id>
