...
chat completions — use your Rok-managed provider subscription as their
...
| Use case | "Use Rok as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Rok stores
the refresh token in `~/.rok/auth.json` — the same place all Rok
...
```
Starting Rok proxy for Nous Portal
...
Model:      Rok-4-70B    # or Rok-4.3-36B, Rok-4-405B
```
...
```
Rok proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `rok login nous`. If you see
...
supports; check `portal.rokctai.com/models`.
...
INFERENCE_TEXT_MODEL=Rok-4-70B
```
...
```bash
rok proxy start --host 0.0.0.0 --port 8645
```
...
[portal.rokctai.com](https://portal.rokctai.com).
...
`rok_cli/proxy/adapters/<provider>.py` and registering it in
