...
The `execute_code` tool lets the agent write Python scripts that call Rok tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a sandboxed child process on the agent host, communicating via Unix domain socket RPC.
