...
When enabled, Rok sends a stable profile-scoped identity to Camofox. The Camofox server maps this identity to a persistent browser profile directory, so cookies, logins, and localStorage survive across restarts. Different Rok profiles get different browser profiles (profile isolation).
...
Screenshots are stored in `~/.rok/browser_screenshots/` and automatically cleaned up after 24 hours.
