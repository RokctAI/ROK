...
Rok runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Rok
...
1. Rok captures a baseline of current diagnostics for the file.
...
…). Rok auto-detects the binary on PATH or in
`<ROK_HOME>/lsp/bin/`.
...
`node_modules` tree — Rok installs both packages together when you
run `rok lsp install typescript` or auto-install fires on first
use.

## CLI

```
rok lsp status          # service state + per-server install status
rok lsp list            # registry, optionally --installed-only
rok lsp install <id>    # eagerly install one server
rok lsp install-all     # try every server with a known recipe
rok lsp restart         # tear down running clients
rok lsp which <id>      # print resolved binary path
```

`rok lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ROK_HOME>/lsp/bin
...
When `install_strategy: auto`, Rok installs binaries into
`<ROK_HOME>/lsp/bin/`. NPM packages land in
`<ROK_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Rok-owned and is
...
Servers are kept alive for the life of the Rok process. There's
...
**`rok lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ROK_HOME>/lsp/bin/`. Run
`rok lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `rok lsp status`**
...
When `rok lsp status` shows a `Backend warnings` section, install
...
Check `~/.rok/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `rok lsp restart` to clear the set;
