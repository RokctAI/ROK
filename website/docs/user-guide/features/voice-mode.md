...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Rok Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Rok Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Rok](/docs/guides/use-voice-mode-with-rok).
...
:::tip
The `~/.rok/` directory and default `config.yaml` are created automatically the first time you run `rok`. You only need to create `~/.rok/.env` manually for API keys.
:::
...
1. Start the CLI with `rok` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.rok/config.yaml` (default: `ctrl+b`).
:::
...
```bash
rok gateway        # Start the gateway (connects to configured platforms)
rok gateway setup  # Interactive setup wizard for first-time configuration
```
...
```bash
rok gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.rok/.env`
...
- Review gateway logs: `tail -f ~/.rok/logs/gateway.log`
