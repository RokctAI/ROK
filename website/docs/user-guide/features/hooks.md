...
Rok has two hook systems that run custom code at key lifecycle points:
...
The gateway ships with a built-in `boot-md` hook that looks for `~/.rok/BOOT.md` on every startup. If the file exists, the agent runs its instructions in a background session. No installation needed — just create the file.
...
1. Check if any cron jobs failed overnight — run `rok cron list`
