...
If you installed with `pip install rok[all]`, the web dependencies are already included.

When you run `rok web` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.
...
When you run `rok update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `rok web` will build it on first launch.
