...
title: "Git Worktrees"
description: "Run multiple Rok agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Rok Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Rok so each session has a clean, isolated working directory.

## Why Use Worktrees with Rok?

Rok treats the **current working directory** as the project root:

- CLI: the directory where you run `rok` or `rok chat`
...
git worktree add ../repo-feature feature/rok-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/rok-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Rok there:

```bash
cd ../repo-feature

# Start Rok in the worktree
rok
```
...
git worktree add ../repo-experiment-a feature/rok-a
git worktree add ../repo-experiment-b feature/rok-b
```
...
- Works on its own branch (`feature/rok-a` vs `feature/rok-b`).
...
- Rok checkpoint data under `~/.rok/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Rok experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/rok-checkpoints-docs`, `feature/rok-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Rok from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `rok -w` (Automatic Worktree Mode)

Rok has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `rok/rok-<hash>`).
...
```bash
rok -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `rok -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Rok session its own clean checkout.
