...
:::warning
Never run two Rok containers against the same data directory simultaneously — session files and memory stores are not designed for concurrent access.
:::
...
Start with `docker compose up -d` and view logs with `docker compose logs -f rok`.
...
- Python 3 with all Rok dependencies (`pip install -e ".[all]"`)
...
The container runs as root by default. If your host `~/.rok/` was created by a non-root user, permissions should work. If you get errors, ensure the data directory is writable:
...
docker exec rok rok version     # Verify version
