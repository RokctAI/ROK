...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/RokctAI/rok-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    rok_meta = (fm.get("metadata") or {}).get("rok") or {}
    tags = rok_meta.get("tags") or []
    related = rok_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `rok skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "Rok ships with a large built-in skill library copied into `~/.rok/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Rok also syncs bundled skills on `rok update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.rok/skills/` tree, it is still shipped with Rok; restore it with `rok skills reset <name> --restore`.",
        "",
